    ...
    namespace UAIMS
    {
        namespace BusinessLogicLayer.BusinessLogic
        {
            ...
            public class FinanceCashBookController
            {
               ...
               // private string _client_constr = System.Configuration.ConfigurationManager.ConnectionStrings["NITPRM"].ConnectionString;

                
                public int AddComp_CashBook(CashBook objCashBook)
                {
                ...
                /// <returns>int</returns>
                public int Create_Tables(CashBook objCashBook)
                {
                    int retStatus = Convert.ToInt32(CustomStatus.Others);
                ...
                public int Create_Synonym_For_Tables(CashBook objCashBook)
                {
                    int retStatus = Convert.ToInt32(CustomStatus.Others);
                ...
                public int PrepareData(string frmdt, string todate, string pcode)
                {
                    int retStatus = Convert.ToInt32(CustomStatus.Others);
                    ...
                    try
                    {
                        ...
                        object ret = objSQLHelper.ExecuteNonQuerySP("PKG_ACC_CASH_REPORT_PREPDATA", objParams, true);
                        if (ret != null)
                        {
                            if (ret.ToString().Equals("-99"))
                                retStatus = Convert.ToInt32(CustomStatus.TransactionFailed);
                ...
                public int PrepareData(string frmdt, string todate, string pcode,string status)
                {
                    int retStatus = Convert.ToInt32(CustomStatus.Others);
                ...
                public int AddRecieptSideEntry(CashBook objCashBook, double opbal,double clbal,double tr,int insup, int isOpbal)
                {
                    int retStatus = Convert.ToInt32(CustomStatus.Others);
