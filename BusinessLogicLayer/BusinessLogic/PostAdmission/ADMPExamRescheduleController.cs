...
{
   public class ADMPExamRescheduleController
    {
        string _UAIMS_constr = string.Empty;
        public ADMPExamRescheduleController()
                {
         ...
         public DataSet GetReschedule(int Degree, string Branch, int ADMBATCH,string ExamSchedule)
         {
             DataSet ds = null;
             SQLHelper objDataAccessLayer = new SQLHelper(_UAIMS_constr);
             ...
             try
             {
                 ...
                 ds = objDataAccessLayer.ExecuteDataSetSP("[dbo].[PKG_ACAD_SP_GETRESCHEDULE]", objParams);
             }
             catch (Exception ex)
             {
                 throw new Exception(ex.Message);
         ...
         public DataSet GetAdmitCardStudent(int ADMBATCH, int ProgramType, int DegreeNo, string branchno, string ExamSchedule)
         {
             ...
             try
             {
                 ...
                 ds = objDataAccessLayer.ExecuteDataSetSP("[dbo].[PKG_GET_STUDENT_LIST_FOR_RESCHEDULE]", objParams);
             }
             catch (Exception ex)
             {
                 throw new Exception(ex.Message);
         ...
         public int Insert_Update_Reschedule(int admbatch, int userNo, string ip_address, int uano, string ExamSchedule, int ScheduleNO,bool IsReschedule,string PrevSchedule, int PrecvScheduleNo) //int rollNo
         {
             //DataSet ds = null;
             ...
             try
             {
                 ...
                 objParams = new SqlParameter[10];
                 {
                     ...
                     objParams[7] = new SqlParameter("@P_PreExamSchedule", PrevSchedule);
                     objParams[8] = new SqlParameter("@P_PreSchedule_no", PrecvScheduleNo);
                     objParams[9] = new SqlParameter("@P_OUTPUT", SqlDbType.Int);
                     objParams[9].Direction = ParameterDirection.Output;
                 };
                 object ret = objSQLHelper.ExecuteNonQuerySP("PKG_INSERT_RESCHEDULE_EXAM", objParams, false);
