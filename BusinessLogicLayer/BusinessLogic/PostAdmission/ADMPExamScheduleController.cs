...
{
   public class ADMPExamScheduleController
    {
       string _UAIMS_constr = string.Empty;
       public ADMPExamScheduleController()
                {
       ...
       public int Add_Update_EntranceSchedule(int ScheduleNo, int admBatch, DateTime date, string time, int DegreeId, string DegreeName, string Branch, int activeStatus, string ipAddress, int createdBy, string Place, string CenterAddress, int Mode, int ORGID,string EndTime,string ReportingTime)
       {
           int status = 0;
           try
           {
               ...
               SqlParameter[] sqlParams = new SqlParameter[] 
                        {
                             ...
                             new SqlParameter("@P_REPORTING_TIME",ReportingTime),
               ...
               else if (obj != null && obj.ToString() != "-99" && obj.ToString() != "-1001" && obj.ToString().Equals("4"))
                   status = Convert.ToInt32(CustomStatus.RecordFound);
               else
