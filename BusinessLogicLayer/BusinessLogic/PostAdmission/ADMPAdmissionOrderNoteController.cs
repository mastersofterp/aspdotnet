...
{
   public class ADMPAdmissionOrderNoteController
    {
        string _UAIMS_constr = string.Empty;
        public ADMPAdmissionOrderNoteController()
                {
         ...
         public DataSet GetStudentForOrderNote(int ADMBATCH, int ProgramType, int DegreeNo, string branchno)
         {
             DataSet ds = null;
             SQLHelper objDataAccessLayer = new SQLHelper(_UAIMS_constr);
             ...
             try
             {
                 ...
                 ds = objDataAccessLayer.ExecuteDataSetSP("[dbo].[PKG_GET_STUDENT_FOR_ADM_ORDER_NOTE]", objParams);
             }
             catch (Exception ex)
             {
                 throw new Exception(ex.Message);
         ...
         public int UpdateMailStatus(int UserNo, int MailUpdateBy)
         {
             int retStatus = Convert.ToInt32(IITMS.UAIMS.CustomStatus.Others);
             try
             {
                 ...
                 objParams = new SqlParameter[3];
                 {
                     ...
                     objParams[1] = new SqlParameter("@P_UA_NO", MailUpdateBy);                   
                     objParams[2] = new SqlParameter("@P_OUTPUT", SqlDbType.Int);
                     objParams[2].Direction = ParameterDirection.Output;
                 };
                 object ret = objSQLHelper.ExecuteNonQuerySP("PKG_ADMP_ADMISSIONORDER_MAIL_LOG", objParams, false);
