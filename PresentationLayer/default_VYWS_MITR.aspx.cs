...
StringComparison.OrdinalIgnoreCase) >= 0))
    ...
    public static List<Packet> GetData(string SLOGINDATA, string IDTOKEN)
    {
        ...
        default_VYWS_MITR objdefault = new default_VYWS_MITR();
        Common objCommon = new Common();
    ...
    protected void btnSendUsernamePassword_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            Regex pass = new Regex("^.*(?=.{6,})(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=]).*$");

            if (!pass.IsMatch(txtEconfirmPass.Text))
            {
                objCommon.DisplayMessage(this.UpdatePanel2, "Password should be at least 6 characters,Must contain at least one  lower case letter, one upper case letter, one digit and one special character", this);
                txtEnewpass.Text = string.Empty;
                txtEconfirmPass.Text = string.Empty;
                return;
            }
