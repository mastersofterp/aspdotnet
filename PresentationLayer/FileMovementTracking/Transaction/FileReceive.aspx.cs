...
public partial class FileMovementTracking_Transaction_FileReceive : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    //Session["idno"] = Session["userno"];          28/10/2022 
                    //Page Authorization
    ...
    protected void btnReceive_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (ViewState["FILE_ID"] == null)   //29-03-2022  gayatri
                {
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        ...
        //                string[] fileName = files[i].ToString().Split('_');
        //                f_name = fileName[0]; // obj.ToString(); 
        //                string userno=fileName[1];
                                            
        //            }
                     
        //        }

        //foreach (ListViewItem lv in lvNewFiles.Items)
        //{
        //   ImageButton del = lv.FindControl("btnDeleteNew") as ImageButton;
        //    HiddenField hdnUserNo = lv.FindControl("hdnUserNo") as HiddenField;


        //    if (hdnUserNo.Value != Session["userno"].ToString())
        //    {
               
        //    }
        //    else
        //    {
        //        string fileCode = lblFCode.Text;
        //        int fileId = Convert.ToInt32(Session["FILE_ID"]);
        ...
        try
        {
            ...
            if (FileUpload1.HasFile)
            {
                if (FileTypeValid(System.IO.Path.GetExtension(FileUpload1.FileName)))
                {
                    ...
                    if (hdnLinkStatus.Value == "1")
                    {
                        path = file + FileUpload1.FileName + "_" + Session["userno"].ToString();       ///24/01/2022;
                    }
                    else
                    {
                        path = file + "\\" + FileUpload1.FileName + "_" + Session["userno"].ToString();    //24/01/2022
    ...
    protected string GetUserNo(object obj)     //24/01/2022
    {
        string f_name = string.Empty;
    ...
    protected void lvNewFiles_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        //24/01/2022
        ImageButton del = e.Item.FindControl("btnDeleteNew") as ImageButton;
        HiddenField hdnUserNo = e.Item.FindControl("hdnUserNo") as HiddenField;
    ...
    protected void rdbList_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (rdbList.SelectedValue == "0")
            {
                ...
                pnlDropDownUserRole.Visible = true; //=-----------02/06/2022
                BindListOfFileMovement(0); //--------------20/06/2022
                pnlList.Visible = true;
            ...
            {
                ddlUserRole.SelectedIndex = 0;     //=-----------02/06/2022
                pnlDropDownUserRole.Visible = false; //=-----------02/06/2022
                BindListOfFileMovement(0);         //------20/06/2022
    ...
    protected void rdbNotApproved_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
          ...
          //  rbtForward.Enabled = true;
            rbtForward.Enabled = false;      //gyatri 25-03-2022
