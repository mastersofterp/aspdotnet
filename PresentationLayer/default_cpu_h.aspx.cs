...
public partial class default_cpu_h : System.Web.UI.Page
{
    /// <summary>
    /// Google Sign In
    /// </summary>
    ...
    protected Boolean ValidateSecurityEmail() //Added by Shrikant W. on 31-08-2023
    {
        try
        {
    ...
    protected Boolean ValidateSecurityMobile() //Added by Shrikant W. on 31-08-2023
    {
        try
        {
...
StringComparison.OrdinalIgnoreCase) >= 0))
    ...
    public static List<Packet> GetData(string SLOGINDATA, string IDTOKEN)
    {
        ...
        default_cpu_h objdefault = new default_cpu_h();
        Common objCommon = new Common();
    ...
    static async Task<int> Execute(string Message, string toEmailId, string sub)
    {
        ...
        try
        {
            ...
            var fromAddress = new System.Net.Mail.MailAddress(dsconfig.Tables[0].Rows[0]["COMPANY_EMAILSVCID"].ToString(), "CPU HAMIRPUR");
            var toAddress = new System.Net.Mail.MailAddress(toEmailId, "");
            System.Net.ServicePointManager.SecurityProtocol = System.Net.SecurityProtocolType.Tls12;
            var apiKey = dsconfig.Tables[0].Rows[0]["SENDGRID_APIKEY"].ToString();
            var client = new SendGridClient(apiKey);
            var from = new EmailAddress(dsconfig.Tables[0].Rows[0]["COMPANY_EMAILSVCID"].ToString(), "CPU HAMIRPUR");
            var subject = sub;
                    ...
                    try
                    {
                        ...
                        if (txtEmail.Text != string.Empty)
                        {
                            try
                            {
                                ...
                                status = objSendEmail.SendEmail(email, message, SUBJECT_OTP + " ERP || OTP for reset password"); //Calling Method
            ...
            try
            {
                ...
                if (txtEmail.Text != string.Empty)
                {
                    try
                    {
                        ...
                        //Task<int> task = Execute(message, txtEmail.Text, "CPU HAMIRPUR ERP || UserName for Candidate");
                        //status = task.Result;
            ...
            try
            {
                ...
                if (txtEmail.Text != string.Empty)
                {
                    try
                    {
                        ...
                        //{
                        //    status = sendEmail(message, txtEmail.Text, "CPUH ERP || OTP for reset password");
                        //}
                        //else if (email_type == "2" && email_type != "")
                        //{
                        //    Task<int> task = Execute(message, txtEmail.Text, "CPUH ERP || OTP for reset password");
                        //    status = task.Result;
                        //}
                        //if (email_type == "3" && email_type != "")
                        //{
                        //    status = OutLook_Email(message, txtEmail.Text, "CPUH ERP || OTP for reset password");
                        //}
    ...
    {
        DataSet ds = objCommon.GetModuleConfig(Convert.ToInt32(4));
        return ds;
    }
