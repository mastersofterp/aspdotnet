...
public partial class default_JECRC_U : System.Web.UI.Page
{
   ...
   // TPController objCompany = new TPController();
            ...
            try
            {
                ...
                if (dr != null)
                {
                    if (dr.Read())
                    {
                        ...
                        Session["SENDGRID_STATUS"] = dr["SENDGRID_STATUS"].ToString();
                        Session.Timeout = 120;
    ...
    /// <summary>
    /// Add by Rita Munde 
    /// This function is used for T&P Activity based on off.......
    protected void GetTPSignupActivity()
    {
        //GetActivityOn();
        DataSet dsActivity = null;

        string alno = (objCommon.LookUp("ACCESS_LINK", "AL_NO", "al_url = 'TP_Reg_form.aspx'"));
        int pageno = Convert.ToInt32(alno);

        //---server page no....
        //int pageno = 2188;
        //  int pageno = 2294;
        //dsActivity = objCompany.CheckActivityON(pageno);

        //if (dsActivity != null)
        //{
        //    if (Convert.ToInt32(dsActivity.Tables[0].Rows[0][0]) == 0)
        //    {
        //        tpSignupLink.Visible = false;
        //    }
        //    else
        //    {
        //        tpSignupLink.Visible = true;
        //    }
        //}
        //else
        //{
...
StringComparison.OrdinalIgnoreCase) >= 0))
    ...
    public static List<Packet> GetData(string SLOGINDATA, string IDTOKEN)
    {
        ...
        default_JECRC_U objdefault = new default_JECRC_U();
        Common objCommon = new Common();
...

        {
            try
            {
                ...
                if (txtEmail.Text != string.Empty)
                {
                    try
                    {
                        if (Session["SENDGRID_STATUS"].ToString() == "1")
                        {
                            //Task<int> task = Execute(message, txtEmail.Text, "ERP || Password for Candidate");
                            //status = task.Result;
                        ...
                        {
                            status = sendEmail(message, txtEmail.Text, SUBJECT_OTP + " ERP || Password for Candidate");
                        }
