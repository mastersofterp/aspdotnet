...
public partial class DefineCounter : System.Web.UI.Page
{
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    // Fill Dropdown lists                
                    this.objCommon.FillDropDownList(ddlCounterUser, "USER_ACC", "UA_NO", "UA_FULLNAME", "UA_TYPE IN (1, 4,5,8)", "UA_FULLNAME");
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                objCommon.DisplayMessage(this, "Please select atleast one receipt permission for counter user.", this.Page);
            ...
            if (ViewState["action"] != null)
            {
                ...
                if (cs.Equals(CustomStatus.Error) || cs.Equals(CustomStatus.TransactionFailed)){
                    objCommon.DisplayMessage(this.updcounter, "Record Already Exists!", this.Page);
                    return;
                }
                else
                    this.ShowAllCounters();
