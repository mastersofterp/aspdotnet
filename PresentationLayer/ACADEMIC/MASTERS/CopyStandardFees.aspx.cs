...
public partial class ACADEMIC_MASTERS_CopyStandardFees : System.Web.UI.Page
{
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    // this._objCommon.FillDropDownList(ddlColgCopy, "ACD_COLLEGE_MASTER", "COLLEGE_ID", "ISNULL(COLLEGE_NAME,'')+(CASE WHEN LOCATION IS NULL THEN '' ELSE ' - 'END) +ISNULL(LOCATION,'') COLLEGE_NAME ", "COLLEGE_ID IN(" + Session["college_nos"] + ") AND COLLEGE_ID > 0", "COLLEGE_ID");
    ...
    private void CheckPageAuthorization()
    {
        if (Request.QueryString["pageno"] != null)
        {
            ...
            {
                Response.Redirect("~/notauthorized.aspx?page=CopyStandardFees.aspx");
            }
        }
        else
        {
    ...
    private void FillListbox(string recieptCode, int degreeNo, int branchno, int batchNo, int payTypeNo)
    {
        try
        {
            StandardFeeController feeController = new StandardFeeController();
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddlReceiptType.SelectedIndex > 0 && ddlPaymentType.SelectedIndex > 0)
            {
                this.DisplayFeeName();
    ...
    private void DisplayFeeDefinitionGrid()
    {
        try
        {
            ...
            DataSet ds = feeController.GetFeeDefinitionForBulk(ddlReceiptType.SelectedValue, ddlSchClg.SelectedValue, ddlDegree.SelectedValue, ddlBranch.SelectedValue, ddlBatch.SelectedValue, ddlPaymentType.SelectedValue);
    ...
    private int UpdateStandardFeeItem_Copy(ListViewDataItem dataRow, int feeCatNo)
    {
        int output = 0;//*********
        ...
        try
        {
            ...
            feeItem.FeeDesc = lblFeeSNameCopy.Text; //((HiddenField)dataRow.FindControl("hidFeeDesc")).Value;
            //feeItem.RecieptCode = ((HiddenField)dataRow.FindControl("hidRecieptCode")).Value;
            feeItem.RecieptCode = ddlRecieptCopy.SelectedValue;
            ...
            feeItem.BatchNo = Convert.ToInt32(ddlBatchCopy.SelectedValue);
            ...
            feeItem.DegreeNo = Convert.ToInt32(ddlDegreeCopy.SelectedValue);
            ...
            feeItem.Branchno = Convert.ToInt32(ddlBranchCopy.SelectedValue);
            ...
            feeItem.PaymentTypeNo = Convert.ToInt32(ddlPaymentCopy.SelectedValue);
            ...
            if (feeItem.DegreeNo == 0 && feeItem.Branchno == 0 && feeItem.BatchNo == 0 && feeItem.PaymentTypeNo == 0)//************
            {
                ...
                if (feeItem.FeeHead == "F1")
                {
                    ...
                    feeItem.PaymentTypeNo = Convert.ToInt32(ddlPaymentCopy.SelectedValue);
                    feeItem.FeeCatNo = feeCatNo;
                    feeItem.FeeDesc = lblFeeSNameCopy.Text;
                }
                else
                {
            ...
            feeItem.CollegeId = Convert.ToInt32(ddlColgCopy.SelectedValue);
            ...
            int FeeDesc = Convert.ToInt32(_objCommon.LookUp("ACD_STANDARD_FEES", "COUNT(1)", "RECIEPT_CODE='" + ddlRecieptCopy.SelectedValue + "' AND BATCHNO=" + ddlBatchCopy.SelectedValue + " AND DEGREENO=" + ddlDegreeCopy.SelectedValue + "AND BRANCHNO=" + ddlBranchCopy.SelectedValue + "AND PAYTYPENO=" + ddlPaymentCopy.SelectedValue + "AND COLLEGE_ID=" + ddlColgCopy.SelectedValue));
            if (FeeDesc > 0)
            {
                int feeCatno = Convert.ToInt32(_objCommon.LookUp("ACD_STANDARD_FEES", "FEE_CAT_NO", "RECIEPT_CODE='" + ddlRecieptCopy.SelectedValue + "' AND BATCHNO=" + ddlBatchCopy.SelectedValue + " AND DEGREENO=" + ddlDegreeCopy.SelectedValue + "AND BRANCHNO=" + ddlBranchCopy.SelectedValue + "AND PAYTYPENO=" + ddlPaymentCopy.SelectedValue + "AND COLLEGE_ID=" + ddlColgCopy.SelectedValue));
                feeItem.FeeCatNo = feeCatno;
            }
            output = feeController.MaintainStandardFeeItemForCopy(ref feeItem);
            if (output == -99)
            {
                return output;
            }
    ...
    protected void btnReport_Click(object sender, EventArgs e)
    {
        if (ddlBatch.SelectedIndex > 0)
        {
            if (ddlPaymentType.SelectedIndex > 0)
            {
                ShowReport("Standard Fees", "rptCopyStandaradFees.rpt");
            }
            else
            {
                _objCommon.DisplayMessage("Please Select Payment Type", Page);
        ...
        {
            _objCommon.DisplayMessage("Please Select Batch", Page);
            ddlBatch.Focus();
    ...
    private void ShowReport(string reportTitle, string rptFileName)
    {
        try
        {
            ...
            //url += "&param=@P_COLLEGE_CODE=" + Session["colcode"].ToString() + ",@UserName=" + Session["username"].ToString() + ",@P_RECIEPT_CODE=" + ddlReceiptType.SelectedValue + ",@P_DEGREENO=" + ddlDegree.SelectedValue + ",@P_BATCHNO=" + ddlBatch.SelectedValue + ",@P_PAYTYPENO=" + ddlPaymentType.SelectedValue + ",@P_RECEIPT=" + ddlReceiptType.SelectedItem.Text.ToString() + ",@P_BATCHNAME=" + ddlBatch.SelectedItem.Text.ToString() + ",@P_PAYMENT=" + ddlPaymentType.SelectedItem.Text.ToString();

            //url += "&param=@P_COLLEGE_CODE=" + Session["colcode"].ToString() + ",@P_RECIEPT_CODE=" + ddlReceiptType.SelectedValue + ",@P_DEGREENO=" + ddlDegree.SelectedValue + ",@P_BATCHNO=" + ddlBatch.SelectedValue + ",@P_PAYTYPENO=" + ddlPaymentType.SelectedValue +",@P_COLLEGE_ID=" + ddlSchClg.SelectedValue;
            url += "&param=@P_COLLEGE_CODE=" + Session["colcode"].ToString() + ",@P_RECIEPT_CODE=" + ddlRecieptCopy.SelectedValue + ",@P_DEGREENO=" + ddlDegreeCopy.SelectedValue + ",@P_BATCHNO=" + ddlBatchCopy.SelectedValue + ",@P_PAYTYPENO=" + ddlPaymentCopy.SelectedValue + ",@P_COLLEGE_ID=" + ddlColgCopy.SelectedValue + ",@P_BRANCHNO=" + ddlBranchCopy.SelectedValue;
    ...
    protected void ddlRecieptCopy_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        //ddlColgCopy.Items.Clear();
        //ddlColgCopy.Items.Add(new ListItem("Please Select", "0"));
