...
public partial class Academic_FeeCollection : System.Web.UI.Page
        ...
        try
            ...
            if (!Page.IsPostBack)
                ...
                if (Session["userno"] == null || Session["username"] == null ||
                    ...
                    //if (Session["Newvalue"] == "1" && Session["Enrollno"] != null)
                    //{
                    //    txtEnrollNo.Text = Session["Enrollno"].ToString();
                    //    this.objCommon.FillDropDownList(ddlSemester, "ACD_SEMESTER", "SEMESTERNO", "SEMESTERNAME", "SEMESTERNO > 0", "SEMESTERNO");
                    //    ddlSemester.SelectedValue = objCommon.LookUp("ACD_STUDENT", "ISNULL(SEMESTERNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR APPLICATIONID='" + txtEnrollNo.Text + "'");
                    ...
                    this.objCommon.FillDropDownList(ddlBankT, "ACD_BANK", "BANKNO", "BANKNAME", "", "BANKNAME");
                    ...
                    /*************************************Added by Dileep Kare on 01.07.2022 ******************************************/
        ...
        try
            ...
            if (txtEnrollNo.Text.Trim() != string.Empty)
                ...
                if (Convert.ToInt32(Session["OrgId"]) == 1)
                    {
                    IDNO = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR APPLICATIONID='" + txtEnrollNo.Text + "'") == "" ? "0" : objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text.Trim() + "' OR APPLICATIONID='" + txtEnrollNo.Text.Trim() + "'"));
                    }
                else
                    {
                    IDNO = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR REGNO='" + txtEnrollNo.Text + "'") == "" ? "0" : objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text.Trim() + "' OR REGNO='" + txtEnrollNo.Text.Trim() + "'"));
                    }
                if (IDNO == 0)
                    {
    ...
    private void DisplayInformation(int studentId)
        ...
        try
            ...
            if (txtPayType.Text.Equals("D"))
                {
                txtPayType.Text = "D";
                divDDDetails.Style.Add("display", "block");
                divCredit.Style.Add("display", "none");
                divTrans.Style.Add("display", "none");
                // txtDDNo.Focus();
                ddlBank.SelectedValue = "250";
                txtTotalAmount.Focus();
                txtPayType.Focus();
                }
            else if (txtPayType.Text.Equals("T"))
                {
                divCredit.Style.Add("display", "block");
                divTrans.Style.Add("display", "block");
            ...
            string SchAmt = (objCommon.LookUp("ACD_STUDENT_SCHOLERSHIP", "ISNULL(SCHL_AMOUNT,0)", "IDNO=" + Convert.ToInt32(studentId) + "AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"])));
            string SchAdjAmt = (objCommon.LookUp("ACD_STUDENT_SCHOLERSHIP", "ISNULL(SCH_ADJ_AMT,0)", "IDNO=" + Convert.ToInt32(studentId) + "AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"])));
    ...
    private void PopulateFeeItemsSection(int semesterNo, DateTime TransDate)
        ...
        try
            ...
            if (status != -99 && ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                ...
                //    RecieptCode == "PGF" || RecieptCode == "BMF" || RecieptCode == "BHE" || RecieptCode == "PDF" || RecieptCode == "UNG" || RecieptCode == "MIS" || RecieptCode == "OF")
    ...
    protected void ddlSemester_SelectedIndexChanged(object sender, EventArgs e)
        ...
        try
            ...
            if (ddlSemester.SelectedIndex > 0)
                this.PopulateFeeItemsSection(Convert.ToInt32(ViewState["semesterno"]), System.DateTime.Now);
            }
        catch (Exception ex)
            {
            if (Convert.ToBoolean(Session["error"]) == true)
    ...
    protected void btnSaveDD_Info_Click(object sender, EventArgs e)
        ...
        try
            ...
            //int count1 = Convert.ToInt32(objCommon.LookUp("ACD_DCR_TRAN", "count(1)", "DD_NO='" + txtDDNo.Text +"' AND DD_BANK='" + ddlBank.SelectedItem.Text.Trim() + "'"));
            ...
            if (Session["DD_Info"] != null && ((DataTable)Session["DD_Info"]) != null)
                {
                int count = Convert.ToInt32(objCommon.LookUp("ACD_DCR_TRAN", "count(DD_NO)", "DD_NO='" + txtDDNo.Text + "' AND DD_BANK='" + ddlBank.SelectedItem.Text.Trim() + "'"));

                //if (count > 0)
                //{
                //    objCommon.DisplayUserMessage(updFee, "Same DD number is already exists !!!!!", this.Page);
                //    return;
                //}
                //else
                //{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
        ...
        try
            ...
            // COMMENTED HERE ON 03122021

            //if ((ddlSemester.SelectedIndex > 0 && ddlSemester.SelectedValue != string.Empty))
            //{
            //    objCommon.DisplayMessage(this.Page, "Semester found null Please Reprocess the Payment", this.Page);
            //    return;
            //}
            ...
            // COMMENTED ENDS HERE ON 03122021
    ...
    private void SaveFeeCollection()
        ...
        try
            ...
            if (feeController.SaveFeeCollection_Transaction_RCPIT(ref dcr, chk, ViewState["ipAddress"].ToString(), Convert.ToInt32(Session["OrgId"])))
                {
                ...
                //{
                //    // this.ShowReport("FeeCollectionReceiptForCash.rpt", dcr.DcrNo, dcr.StudentId, "1");
                //    this.ShowReport_ForCash("FeeCollectionReceiptForCash.rpt", dcr.DcrNo, dcr.StudentId, "1");
                ...
                if (enroll.Equals(""))
                    {
                    //  objCommon.DisplayUserMessage(this.updFee, "Transaction Saved Successfully.", this.Page);
                    Session["Newvalue"] = null;
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "redirect script", "alert('Transaction Saved Successfully.');", true);
                    //location.href='FeeCollectionOptions.aspx?';", true);
                    }
                ...
                //}
                    ...
                    {
                    //  objCommon.DisplayUserMessage(this.updFee, "Transaction Saved Successfully", this.Page);
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "redirect script", "alert('Transaction Saved Successfully.');", true);               //location.href='FeeCollectionOptions.aspx';", true);
                    Session["Newvalue"] = null;
    ...
    private DailyCollectionRegister Bind_FeeCollectionData()
        ...
        try
            ...
            // dcr.SemesterNo = ((ddlSemester.SelectedIndex > 0 && ddlSemester.SelectedValue != string.Empty) ? Int32.Parse(ddlSemester.SelectedValue) : 1);
            int studsem = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT", "SEMESTERNO", "IDNO=" + dcr.StudentId));  // ADDED BY NARESH BEERLA AS PER ANKUSH SIR 04122021
            dcr.SemesterNo = ((Convert.ToInt32(ViewState["semesterno"]) > 0 && ViewState["semesterno"] != string.Empty) ? Convert.ToInt32(ViewState["semesterno"]) : studsem);
            ...
            //if (txtPayType.Text.Trim() == "T")
            //{
            //    dcr.DemandDraftAmount = Convert.ToDouble(txtDDAmount.Text);
            //}            
            ...
            if (Session["Newvalue"] == "1")
                {
                dcr.PaymentModeCode = ViewState["paymentmode"].ToString();
                // ViewState["PaymentMode"] = Request.QueryString["PaymentMode"].ToString();

                dcr.IsCancelled = (ViewState["paymentmode"].ToString() == "C" ||
               GetViewStateItem("paymentmode") == "A") ? false : true;
                ...
                dcr.IsReconciled = (ViewState["paymentmode"].ToString() == "C" ||
            ...
            dcr.PaymentType = txtPayType.Text.Trim();
            ...
            dcr.BankId = Convert.ToInt32(ddlBankT.SelectedValue);
    ...
    private double GetCashAmount(double totalDDAmt)
        ...
        try
            ...
            //else if (txtPayType.Text.Trim() == "T")
            //{
            //    cashAmount = Convert.ToDouble(txtTotalDDAmount.Text.Trim());
            //}
            else
                {
    ...
    private double GetTotalDDAmountAndSetCompleteDetails(ref DemandDrafts[] paidDemandDrafts)
        ...
        try
            ...
            if (txtPayType.Text.Trim() == "D" || txtPayType.Text.Trim() == "T" || txtPayType.Text.Trim() == "C")
                {
                if (Session["DD_Info"] != null && ((DataTable)Session["DD_Info"]) != null)
                    {
                ...
                //else if (txtPayType.Text.Trim() == "T")
                //{
                //    totalDdAmt = Convert.ToDouble(txtTotalDDAmount.Text.Trim());
                //}
                ...
                if (Convert.ToString(ViewState["paymentmode"]) == "C")
                    ...
                    if (Convert.ToInt32(Session["OrgId"]) == 6)
                        ...
                        if (ReportFlag == 1)
                            ...
                            url += "ClgID=" + Session["colcode"].ToString() + "&UA_NAME=" + Session["UAFULLNAME"].ToString() + "&Idno=" + Int32.Parse(GetViewStateItem("StudentId")) + "&DcrNo=" + Int32.Parse(GetViewStateItem("DcrNo")) + "&Cancel=" + Convert.ToInt32(Session["CANCEL_REC"]);
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('" + url + "');", true);

                            //  // Above Code added by Rohit M. on dated 17.06.2023 
                            }
                        else
                            {

                            this.ShowReport_ForCash("FeeCollectionReceiptForCash_RCPIPER.rpt", Int32.Parse(GetViewStateItem("DcrNo")), Int32.Parse(GetViewStateItem("StudentId")), "1", Convert.ToInt32(Session["CANCEL_REC"]));
                            }
                        }
                    else
                        {
                        if (ReportFlag == 1)
                            ...
                            {
                            //this.ShowReport("FeeCollectionReceiptForCash.rpt", Int32.Parse(GetViewStateItem("DcrNo")), Int32.Parse(GetViewStateItem("StudentId")), "1");
                            this.ShowReport_ForCashRCPIT("FeeCollectionReceiptForCash.rpt", Int32.Parse(GetViewStateItem("DcrNo")), Int32.Parse(GetViewStateItem("StudentId")), "1", Session["UAFULLNAME"].ToString(), Convert.ToInt32(Session["CANCEL_REC"]));
                            }
                        }
                    }
                else
                    {
                    this.ShowReport("FeeCollectionReceipt.rpt", Int32.Parse(GetViewStateItem("DcrNo")), Int32.Parse(GetViewStateItem("StudentId")), "1", Convert.ToInt32(Session["CANCEL_REC"]));
                    }
    ...
    protected void btnPrintReceipt_Click(object sender, EventArgs e)
        ...
        try
                ...
                if (Convert.ToString(ViewState["paymentmode"]) == "C")
                    ...
                    if (Convert.ToInt32(Session["OrgId"]) == 6)
                        ...
                        if (ReportFlag == 1)
                            ...
                            string url = Request.Url.ToString().Substring(0, (Request.Url.ToString().ToLower().IndexOf("academic")));
                            url += "Reports/Academic/Fees/RCPIPERReceipt.html?";
                            url += "ClgID=" + Session["colcode"].ToString() + "&UA_NAME=" + Session["UAFULLNAME"].ToString() + "&Idno=" + Int32.Parse(GetViewStateItem("StudentId")) + "&DcrNo=" + Int32.Parse(btnPrint.CommandArgument) + "&Cancel=" + Convert.ToInt32(Session["CANCEL_REC"]);
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('" + url + "');", true);
                            ...
                            {
                            this.ShowReport_ForCash("FeeCollectionReceiptForCash_RCPIPER.rpt", Int32.Parse(btnPrint.CommandArgument), Int32.Parse(GetViewStateItem("StudentId")), "1", Convert.ToInt32(Session["CANCEL_REC"]));
                            }
                        }
                    else
                        {
                        ...
                        if (ReportFlag == 1)
                            ...
                            {
                            //this.ShowReport("FeeCollectionReceiptForCash.rpt", Int32.Parse(GetViewStateItem("DcrNo")), Int32.Parse(GetViewStateItem("StudentId")), "1");
                            this.ShowReport_ForCashRCPIT("FeeCollectionReceiptForCash.rpt", Int32.Parse(btnPrint.CommandArgument), Int32.Parse(GetViewStateItem("StudentId")), "1", Session["UAFULLNAME"].ToString(), Convert.ToInt32(Session["CANCEL_REC"]));
                            }
                    ...
                    {
                    this.ShowReport("FeeCollectionReceipt.rpt", Int32.Parse(btnPrint.CommandArgument), Int32.Parse(GetViewStateItem("StudentId")), "2", Convert.ToInt32(Session["CANCEL_REC"]));
                    }
            ...
            {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUaimsCommon.ShowError(Page, "Academic_FeeCollection.btnPrintReceipt_Click() --> " + ex.Message + " " + ex.StackTrace);
            else
                objUaimsCommon.ShowError(Page, "Server Unavailable.");
            }
        }

    private void ShowReport(string rptName, int dcrNo, int studentNo, string copyNo, int Cancel)
        {
        try
            {
            //string url = Request.Url.ToString().Substring(0, (Request.Url.ToString().IndexOf("Academic")));
            ...
            url += "&path=~,Reports,Academic," + rptName;
            url += "&param=@P_COLLEGE_CODE=" + Session["colcode"].ToString() + "," + "@P_CANCEL=" + Convert.ToInt32(Session["CANCEL_REC"]) + "," + this.GetReportParameters(dcrNo, studentNo, copyNo);
            divMsg.InnerHtml += " <script type='text/javascript' language='javascript'> try{ ";
            divMsg.InnerHtml += " window.open('" + url + "','Fee_Collection_Receipt','addressbar=no,menubar=no,scrollbars=1,statusbar=no,resizable=yes');";
            divMsg.InnerHtml += " }catch(e){ alert('Error: ' + e.description);}</script>";
    ...
    private void ShowReport_ForCashRCPIT(string rptName, int dcrNo, int studentNo, string copyNo, string Username, int Cancel)
        {
        try
            {
            //string url = Request.Url.ToString().Substring(0, (Request.Url.ToString().IndexOf("Academic")));
            ...
            url += "pagetitle=Fee_Collection_Receipt";
            url += "&path=~,Reports,Academic," + rptName;
            url += "&param=@P_COLLEGE_CODE=" + Session["colcode"].ToString() + "," + "@P_UA_NAME=" + Session["username"].ToString() + "," + "@P_CANCEL=" + Convert.ToInt32(Session["CANCEL_REC"]) + "," + this.GetReportParameters(dcrNo, studentNo, copyNo) + ",username=" + Session["username"].ToString();
    ...
    private void CreateDemandForCurrentFeeCriteria()
        ...
        try
            ...
            feeDemand.SemesterNo = ((Convert.ToInt32(ViewState["semesterno"]) > 0 && ViewState["semesterno"].ToString() != string.Empty) ? Convert.ToInt32(ViewState["semesterno"]) : 1);
    ...
    private void CreateDemandForCurrentFeeCriteriamis()
        ...
        try
            ...
            feeDemand.BranchNo = ((GetViewStateItem("BranchNo") != string.Empty) ? Convert.ToInt32(GetViewStateItem("BranchNo")) : 0);
            feeDemand.DegreeNo = ((GetViewStateItem("DegreeNo") != string.Empty) ? Convert.ToInt32(GetViewStateItem("DegreeNo")) : 0);
            feeDemand.SemesterNo = ((Convert.ToInt32(ViewState["semesterno"]) > 0 && ViewState["semesterno"].ToString() != string.Empty) ? Convert.ToInt32(ViewState["semesterno"]) : 0);
            feeDemand.AdmBatchNo = ((GetViewStateItem("AdmBatchNo") != string.Empty) ? Convert.ToInt32(GetViewStateItem("AdmBatchNo")) : 0);
            feeDemand.ReceiptTypeCode = GetViewStateItem("ReceiptType");
            feeDemand.PaymentTypeNo = ((GetViewStateItem("PaymentTypeNo") != string.Empty) ? Convert.ToInt32(GetViewStateItem("PaymentTypeNo")) : 0);
            feeDemand.CounterNo = ((GetViewStateItem("CounterNo") != string.Empty) ? Convert.ToInt32(GetViewStateItem("CounterNo")) : 0);
    ...
    private string GetNewReceiptNo()
        ...
        try
            ...
            if (Session["Newvalue"] == "1")
                {
                ds = feeController.GetNewReceiptData(ViewState["paymentmode"].ToString(), Int32.Parse(Session["userno"].ToString()), ViewState["ReceiptType"].ToString());
                }
            else
                {
                ds = feeController.GetNewReceiptData(GetViewStateItem("paymentmode"), Int32.Parse(Session["userno"].ToString()), ViewState["ReceiptType"].ToString());
                }
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                //String reciptCode;
                ...
                FeesSessionStartDate = objCommon.LookUp("REFF", "CONCAT(YEAR(START_YEAR),'-',YEAR (GETDATE()) +1)", "");
                ...
                if (Session["Newvalue"] == "1")
                    {
                    receiptNo = dr["PRINTNAME"].ToString() + "/" + ViewState["paymentmode"] + "/" + ViewState["ReceiptType"].ToString() + "/" + FeesSessionStartDate + "/" + dr["FIELD"].ToString();
                    }
                else
                    {
                    receiptNo = dr["PRINTNAME"].ToString() + "/" + GetViewStateItem("paymentmode") + "/" + ViewState["ReceiptType"].ToString() + "/" + FeesSessionStartDate + "/" + dr["FIELD"].ToString();
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
        ...
        //Response.Redirect(Request.Url.ToString());
        //}
        Response.Redirect("~/Academic/FeeCollectionOptions.aspx", false);
        }
    ...
    protected void chkAllowExcessFee_CheckedChanged(object sender, EventArgs e) //Sunita
        {
        // COMMENTING AS PER ANKUSH SIR ON 04122021
        ...
        // ENDS HERE COMMENTING ON 04122021
        }
    ...
    protected void btnNewFee_Click(object sender, EventArgs e)
        {
        // Response.Redirect("FeeCollectionOptions.aspx");
        Response.Redirect("~/Academic/FeeCollection.aspx", false);
        }
    ...
    private void FeesDetails()
        ...
        try
            ...
            if (txtEnrollNo.Text.Trim() != string.Empty)
                ...
                if (Convert.ToInt32(Session["OrgId"]) == 1)
                    {
                    IDNO = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR APPLICATIONID='" + txtEnrollNo.Text + "'") == "" ? "0" : objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR APPLICATIONID='" + txtEnrollNo.Text + "'"));
                    }
                else
                    {
                    IDNO = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR REGNO='" + txtEnrollNo.Text + "'") == "" ? "0" : objCommon.LookUp("ACD_STUDENT", "ISNULL(IDNO,0)", "ENROLLNO='" + txtEnrollNo.Text + "' OR REGNO='" + txtEnrollNo.Text + "'"));
                    }
                    ...
                    if (ISCounter != 0)
                        ...
                        if (studentId > 0)
                            ...
                            if (semester != 1)
                                ...
                                feedback = "False";//objCommon.LookUp("REFF", "Feedback_Status", "");
    ...
    private void bindlist(string category, string searchtext)
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            ...
            objCommon.DisplayUserMessage(this.Page, "Record Not Found,Kindly Check for Demand is Creates or Not.", this.Page);
    ...
    protected void lnkIdPanel_Click(object sender, EventArgs e)
        ...
        try
            ...
            lblSsem.Text = lblSem.Text;
            int semno = Convert.ToInt32(lblSem.ToolTip);

            if (semno == 1 || semno == 2)
                lblSyear.Text = "First Year";
            else if (semno == 3 || semno == 4)
                lblSyear.Text = "Second Year";
            else if (semno == 5 || semno == 6)
                lblSyear.Text = "Third Year";
            else if (semno == 7 || semno == 8)
                lblSyear.Text = "Fourth Year";
            else if (semno == 9 || semno == 10)
                lblSyear.Text = "Fifth Year";
    ...
    //{
    //int installmentAmount = 0;
    //DataSet dsInstallment = feeController.GetStudentInstallmentDetails(Convert.ToInt32(Session["IDNO"]), Convert.ToInt32(ViewState["semesterno"]), Convert.ToString(ViewState["ReceiptType"]));
    //if (dsInstallment != null && dsInstallment.Tables.Count > 0 && dsInstallment.Tables[0].Rows.Count > 0)
    //{
    ...
    //    installmentAmount = Convert.ToInt32(dt.Rows[0]["INSTALL_AMOUNT"]);// Tables[0].Col["INSTALL_AMOUNT"].ToString());
    //    txtTotalAmount.Text = installmentAmount.ToString();
    //    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "CallMyFunction", "DevideTotalAmount()", true);
