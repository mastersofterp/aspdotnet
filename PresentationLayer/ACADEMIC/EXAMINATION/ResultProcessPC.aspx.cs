...
public partial class ACADEMIC_EXAMINATION_ResultProcessPC : System.Web.UI.Page
{
    ...
    private void PopulateDropDownList()
    {
        try
        {
            ...
            objCommon.FillDropDownList(ddlColg, "ACD_COLLEGE_MASTER C INNER JOIN ACD_COLLEGE_DEGREE_BRANCH CD ON (CD.COLLEGE_ID=C.COLLEGE_ID)", "DISTINCT (C.COLLEGE_ID)", "C.COLLEGE_NAME", "C.COLLEGE_ID > 0 AND CD.UGPGOT IN (" + Session["ua_section"] + ") AND CD.UGPGOT = 3", "C.COLLEGE_ID");  // 
    ...
    protected void btnProcessResult_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                CustomStatus cs = (CustomStatus)objProcessResult.ProcessResultPCAll(Convert.ToInt32(ddlSession.SelectedValue), Convert.ToInt32(ddlScheme.SelectedValue), Convert.ToInt32(ddlSem.SelectedValue), 0, idno, ViewState["ipAddress"].ToString(), Convert.ToInt32(ddlColg.SelectedValue));
    ...
    protected void ddlColg_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            objCommon.FillDropDownList(ddlDegree, "ACD_DEGREE D INNER JOIN ACD_COLLEGE_DEGREE CD ON (D.DEGREENO = CD.DEGREENO) INNER JOIN ACD_COLLEGE_DEGREE_BRANCH CDB ON (CDB.DEGREENO=D.DEGREENO)", "DISTINCT D.DEGREENO", " D.DEGREENAME", " CD.COLLEGE_ID=" + ddlColg.SelectedValue + "AND CDB.UGPGOT IN (" + Session["ua_section"] + ") AND CDB.UGPGOT =3", "D.DEGREENO");
            ddlDegree.Focus();
