...
public partial class ACADEMIC_UnlockM_RE : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ddlStuType.Items.Add(new ListItem("RE MAJOR", "2"));
      
        // Add the ListItem to the DropDownList
        ListItem item = new ListItem("RE MAJOR", "2");
        ddlStuType.Items.Add(item);

        // Set the default selected value
        ddlStuType.SelectedValue = item.Value;

        // Disable the DropDownList
        ddlStuType.Enabled = false;
    }
    #endregion
    ...
    private void FillDropdownList()
    {
        ...
        //objCommon.FillDropDownList(ddlClgname, "ACD_COLLEGE_SCHEME_MAPPING SM INNER JOIN ACD_COLLEGE_DEGREE_BRANCH DB ON (SM.OrganizationId = DB.OrganizationId AND SM.DEGREENO = DB.DEGREENO AND SM.BRANCHNO = DB.BRANCHNO AND SM.COLLEGE_ID = DB.COLLEGE_ID)", "COSCHNO", "COL_SCHEME_NAME", "SM.COLLEGE_ID IN(" + Session["college_nos"] + ") AND COSCHNO>0 AND SM.COLLEGE_ID > 0 AND SM.OrganizationId=" + Convert.ToInt32(System.Web.HttpContext.Current.Session["OrgId"]), "");+
              objCommon.FillDropDownList(ddlClgname, "ACD_COLLEGE_SCHEME_MAPPING", "COSCHNO", "COL_SCHEME_NAME", "COLLEGE_ID IN(" + Session["college_nos"] + ") AND COSCHNO>0 AND COLLEGE_ID > 0 AND SCHEMENO IN (SELECT DISTINCT SCHEMENO FROM ACD_STUDENT_RESULT WHERE ISNULL(ABSENT_LOG,0)=1) AND OrganizationId=" + Convert.ToInt32(System.Web.HttpContext.Current.Session["OrgId"]), "COLLEGE_ID DESC");
       ...
       // objCommon.FillDropDownList(ddlSubType, "ACD_SUBJECTTYPE ", "SUBID", "SUBNAME", "SUBID > 0 AND ISNULL(ACTIVESTATUS,0)=1  ", "SUBID ");
        ...
        if (Convert.ToInt32(Session["OrgId"]) == 3 || Convert.ToInt32(Session["OrgId"]) == 4)   //ADDED BY GAURAV FOR CPUK and H
        {
            ddlStuType.Items.Add(new ListItem("RE MAJOR", "2"));
            #region
            // Add the ListItem to the DropDownList
    ...
    private void UpdateLockStatus(ListViewDataItem dataRow)
    {
        ...
        try
        {
            ...
            string remark = "REMAJOR_"+txtremark.Text;
            ...
            if (chk.Checked == false && chk.Enabled == true)
            {
                ...
                DataSet dsmarkcount = GetMarkEntryNotDonecount_REMAJOR(Convert.ToInt32(ViewState["college_id"]), Convert.ToInt32(ddlSession.SelectedValue), Convert.ToInt32(ViewState["degreeno"]), Convert.ToInt32(ViewState["branchno"]), Convert.ToInt32(ViewState["schemeno"]), Convert.ToInt32(ddlSemester.SelectedValue), Convert.ToInt32(ddlStuType.SelectedValue), Convert.ToInt32(ddlExamType.SelectedValue), Convert.ToInt32(ddlSubType.SelectedValue), Convert.ToInt32(courseno), divSubExamType.Visible == true ? ddlSubExamType.SelectedValue.Trim() : string.Empty, Convert.ToInt32(section));
                ...
                if (lockunlock_status == true)
                {
                    if (dsmarkcount != null && dsmarkcount.Tables[0].Rows.Count > 0)
                    {
                        if (dsmarkcount.Tables[0].Rows[0]["count"].ToString() != "0")
                        {
                            ...
                            LockUnLockMarkEntryByAdmin_REMAJOR(SessionNo, semester, schemeno, examtype, Convert.ToInt32(courseno), Convert.ToInt32(section), Convert.ToInt32(facultynotheory), Convert.ToInt32(facultynopractical), Convert.ToInt32(lockunlock), ipAddress, Convert.ToInt32(Session["userno"]), divSubExamType.Visible == true ? ddlSubExamType.SelectedValue : string.Empty, remark); 
                    }
                    else
                        LockUnLockMarkEntryByAdmin_REMAJOR(SessionNo, semester, schemeno, examtype, Convert.ToInt32(courseno), Convert.ToInt32(section), Convert.ToInt32(facultynotheory), Convert.ToInt32(facultynopractical), Convert.ToInt32(lockunlock), ipAddress, Convert.ToInt32(Session["userno"]), divSubExamType.Visible == true ? ddlSubExamType.SelectedValue : string.Empty, remark); 
                }
    ...
    private void BindExam()
    {
        try
        {
            DataSet dsCourse = null;
                ...
                dsCourse = GetCoursesForLockUnlock_REMAJOR(Convert.ToInt32(ViewState["college_id"]), Convert.ToInt32(ddlSession.SelectedValue), Convert.ToInt32(ViewState["degreeno"]), Convert.ToInt32(ViewState["branchno"]), Convert.ToInt32(ViewState["schemeno"]), Convert.ToInt32(ddlSemester.SelectedValue), Convert.ToInt32(ddlStuType.SelectedValue), Convert.ToInt32(ddlExamType.SelectedValue), Convert.ToInt32(ddlSubType.SelectedValue), divSubExamType.Visible == true ? ddlSubExamType.SelectedValue.Trim() : string.Empty);
    ...
    protected void btnShowStatus_Click(object sender, EventArgs e)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            GVStatus.DataBind();
            string attachment = "attachment; filename=" + degree.Replace(" ", "_") + "_" + scheme.Replace(" ", "_") + "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xls";
    ...
    public void SendSMS(string MOBILENO, string MSG)
    {
        ...
        {
            //Call Send SMS API
            string sendSMSUri = "https://bulksms.ezzysms.in/sendhttp.php";
    ...
    protected void ddlSemester_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        objCommon.FillDropDownList(ddlSubType, "ACD_SUBJECTTYPE A INNER JOIN ACD_STUDENT_RESULT B ON (A.SUBID=B.SUBID) ", "DISTINCT A.SUBID", "A.SUBNAME", "A.SUBID > 0 AND ISNULL(A.ACTIVESTATUS,0)=1 AND ISNULL(ABSENT_LOG,0)=1 ", "A.SUBID ");
        ...
        try
        {
            ...
            if (ddlClgname.SelectedIndex > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0 && ds.Tables[0] != null)
                {
                    ...
                    objCommon.FillDropDownList(ddlSession, "ACD_SESSION_MASTER", "SESSIONNO", "SESSION_PNAME", "COLLEGE_ID = " + Convert.ToInt32(ViewState["college_id"]) + " AND SESSIONNO IN (SELECT DISTINCT SESSIONNO FROM ACD_STUDENT_RESULT WHERE ISNULL(ABSENT_LOG,0)=1)", "SESSIONNO DESC");
    ...
    protected void btnlock_Click(object sender, EventArgs e)
    {
        ...
        else
        {
             try
            {
                ...
                string attachment = "attachment; filename=" + "LOCKUNLOCK" + "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xls";
                Response.ClearContent();
                Response.AddHeader("content-disposition", attachment);
                ...
                string sp_procedure = "PKG_GET_LOCK_UNLOCK_REMAJOR_REPORT";
    ...
    #region added controller 

    private DataSet GetCoursesForLockUnlock_REMAJOR(int CollegeID, int SessionNo, int degreeno, int branchno, int schemeno, int semesterno, int studenttype, int Int_Ext, int subid, string subexam)
    {
        DataSet ds = null;
        try
        {
            ...
            ds = objSQLHelper.ExecuteDataSetSP("PKG_ACD_GET_COURSES_BY_SCHEME_AND_OPERATOR_REMAJOR", objParams);
    ...
    public DataSet GetMarkEntryNotDonecount_REMAJOR(int CollegeID, int SessionNo, int degreeno, int branchno, int schemeno, int semesterno, int studenttype, int Int_Ext, int subid, int courseno, string subexam, int sectionno)
    {
        DataSet ds = null;
        try
        {
            ...
            ds = objSQLHelper.ExecuteDataSetSP("PKG_CHK_MARK_ENTRY_DONE_REMAJOR", objParams);
    ...
    private int LockUnLockMarkEntryByAdmin_REMAJOR(int sessionno, int semester, int schemeno, int examtype, int courseno, int section, int facultynotheory, int facultynopractical, int lockunlock, string ipaddress, int lock_by, string subexamtype, string remark)
    {
        ...
        try
        {
            ...
            object ret = objSQLHelper.ExecuteNonQuerySP("PKG_MARK_ENTRY_LOCK_UNLOCK_REMAJOR", objParams, false);
            if (ret != null)
            {
