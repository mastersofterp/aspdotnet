...
public partial class ACADEMIC_MentorMentee_Meeting_Schedule : System.Web.UI.Page
{
    ...
    public void sendmail(string fromEmailId, string fromEmailPwd, string Sub, string body, string userid)
    {
        try
        {
            ...
            dsCommittee = objCommon.FillDropDown("ACD_MEETING_COMITEE", "ID, code", "NAME", "ID=" + Convert.ToInt32(ddlCommitee.SelectedValue) + "", "");
            DataSet ds = objCommon.FillDropDown("ACD_MEETING_SCHEDULE A INNER JOIN TBL_MM_VENUE V ON (A.VENUEID = V.PK_VENUEID)", "distinct A.MEETING_CODE, A.AGENDANO", "A.MEETINGDATE, A.MEETINGTIME, V.VENUE, A.LAST_DATE", "A.FK_MEETING=" + Convert.ToInt32(ddlCommitee.SelectedValue) + " AND A.MEETING_CODE='" + txtcode.Text.Trim() + "'", "");
            ...
            for (int j = 0; j < idno.Length; j++)
            {
                ...
                MemberEmailId = objCommon.LookUp("ACD_MEETING_MENBERDETAILS MD INNER JOIN ACD_MEETING_MENBERDETAILS RM ON (MD.PK_CMEMBER = RM.FK_MEMBER)", "ISNULL(T_EMAIL,' ') AS T_EMAIL", "MD.USERID=" + idno[j] + " AND RM.FK_COMMITEE=" + Convert.ToInt32(ddlCommitee.SelectedValue));
                ...
                DataSet dsAgendaList = objCommon.FillDropDown("ACD_MEETING_SCHEDULE", "PK_AGENDA, MEETING_CODE, AGENDANO", "ROW_NUMBER() OVER(ORDER BY PK_AGENDA ASC) SrNo, AGENDATITAL", "FK_MEETING=" + Convert.ToInt32(ddlCommitee.SelectedValue) + " AND MEETING_CODE='" + txtcode.Text.Trim() + "'", "");
    ...
    public void sendmailToCancelMeeting(string fromEmailId, string fromEmailPwd, string toEmailId, string Sub, string body, string userid)
    {
        try
        {
            ...
            DataSet dsCommittee = null;
            dsCommittee = objCommon.FillDropDown("ACD_MEETING_COMITEE", "ID, code", "NAME", "ID=" + Convert.ToInt32(ddlCommitee.SelectedValue) + "", "");
            DataSet ds = objCommon.FillDropDown("ACD_MEETING_SCHEDULE", " distinct MEETING_CODE, AGENDANO", "MEETINGDATE, MEETINGTIME, VENUE", "FK_MEETING=" + Convert.ToInt32(ddlCommitee.SelectedValue) + " AND MEETING_CODE='" + txtcode.Text.Trim() + "'", "");
