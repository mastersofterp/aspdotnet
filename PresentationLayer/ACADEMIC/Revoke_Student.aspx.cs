...
public partial class ACADEMIC_Revoke_Student : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            else
            {
                ...
                objCommon.FillDropDownList(ddlProgrammeType, "ACD_COLLEGE_MASTER", "DISTINCT COLLEGE_ID", "COLLEGE_NAME", "ISNULL(ACTIVESTATUS,0)=1", "COLLEGE_ID");
    ...
    protected void btnShowStud_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int branch = ddlBranch.SelectedIndex > 0 ? Convert.ToInt32(ddlBranch.SelectedValue) : 0;
            int college = ddlProgrammeType.SelectedIndex > 0 ? Convert.ToInt32(ddlProgrammeType.SelectedValue) : 0;

            DataSet ds = objFetchData.GetApplicantUserRevokeList(Convert.ToInt32(ddlAdmbatch.SelectedValue), college, degree, branch);
    ...
    protected void btnCompleteDetail_Click(object sender, EventArgs e)
    {
        try
        {
            try
            {
                ...
                DataSet ds = objFetchData.GetApplicantsCompleteDetails(Convert.ToInt32(ddlAdmbatch.SelectedValue), programmeType, Convert.ToInt32(1), degreeNo);
                if (ds.Tables[0].Rows.Count > 0)
                {
    ...
    protected void btnAdmissionCount_Click(object sender, EventArgs e)
    {
        ...
        int degreeNo = ddlDegree.SelectedIndex > 0 ? Convert.ToInt32(ddlDegree.SelectedValue) : 0;
        int Application_Status = Convert.ToInt32(1);
    ...
    protected void btnStudDetails_Click(object sender, EventArgs e)
    {
...

        {
            ...
            dsStudData = objFetchData.GetStudentCompleteDetailsNew(Convert.ToInt32(ddlDegree.SelectedValue), admbatch, Progtype, Convert.ToInt32(1), Convert.ToInt32(ddlBranch.SelectedValue));
        }
        GridView gv = new GridView();
    ...
    protected void btnClose_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataSet ds = objFetchData.GetApplicantUserListNew_JECRC(Convert.ToInt32(ddlAdmbatch.SelectedValue), programmeType, Convert.ToInt32(1), degree);
            if (ds.Tables[0].Rows.Count > 0)
            {
                lvStudent.DataSource = ds;
                lvStudent.DataBind();
    ...
    protected void btnSubmitPopup_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (csParti.Equals(CustomStatus.RecordUpdated))//
            {
                ...
                DataSet ds = objFetchData.GetApplicantUserListNew_JECRC(Convert.ToInt32(ddlAdmbatch.SelectedValue), programmeType, Convert.ToInt32(1), degreegrd);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    lvStudent.DataSource = ds;
                    lvStudent.DataBind();
    ...
    protected void ddlProgrammeType_SelectedIndexChanged1(object sender, EventArgs e)
    {
        try
        {
            if (ddlProgrammeType.SelectedIndex > 0)
            {
                objCommon.FillDropDownList(ddlDegree, " ACD_DEGREE D INNER JOIN ACD_COLLEGE_DEGREE_BRANCH CDB  ON (D.DEGREENO = CDB.DEGREENO)", "DISTINCT D.DEGREENO", "D.DEGREENAME", "CDB.COLLEGE_ID=" + Convert.ToInt32(ddlProgrammeType.SelectedValue) + " AND D.DEGREENO > 0", "D.DEGREENAME");
                ddlDegree.Focus();
            }
            else
            {
    ...
    protected void btnRevoke_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string remark = "Student has revoke by " + uano + " on " + date +"";
            int count = 0;
            foreach (ListViewDataItem dataitem in lvStudent.Items)
            {
            ...
            if (count == 0)
            {
                objCommon.DisplayMessage(this.Page, "Please select atleast one student to revoke.", this.Page);
                return;
            }
            else
            {
                foreach (ListViewDataItem item in lvStudent.Items)
                {
                    ...
                    Label idno = item.FindControl("Label5") as Label;
                    int u_idno = Convert.ToInt32(idno.Text);
                    if (chek.Checked)
                    {
                        int cs = objFetchData.UpdateRevokeStudStatus(u_idno,remark);
                    }
                }
                int programmeType = ddlProgrammeType.SelectedIndex > 0 ? Convert.ToInt32(ddlProgrammeType.SelectedValue) : 0;
                int degree = ddlDegree.SelectedIndex > 0 ? Convert.ToInt32(ddlDegree.SelectedValue) : 0;
                ...
                objCommon.DisplayMessage(this.Page, "Student Revoked Sucessfully.", this.Page);
