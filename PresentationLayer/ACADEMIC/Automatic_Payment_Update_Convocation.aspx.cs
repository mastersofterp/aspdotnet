...
public partial class ACADEMIC_Automatic_Payment_Update : System.Web.UI.Page
{
    ...
    public int SubmitData(string Order_ID, int IDNO, string IsValid, string Receipt_Code, string TransactionDate, string ApTransactionID, string PayStatus, string Message,string transid)
    {
        int retStatus1 = 0;
        ...
        try
        {
            ...
            SqlParameter[] sqlparam = null;
            {
                ...
                sqlparam[5] = new SqlParameter("@P_TRANSID", transid);
                sqlparam[6] = new SqlParameter("@P_PAY_STATUS", PayStatus);
                sqlparam[7] = new SqlParameter("@P_MESSAGE", Message);
                sqlparam[8] = new SqlParameter("@P_APTRANSACTIONID", ApTransactionID);
                sqlparam[9] = new SqlParameter("@P_OUTPUT", SqlDbType.Int);
                sqlparam[9].Direction = ParameterDirection.Output;
                string idcat = sqlparam[9].Direction.ToString();
            };
            object studid = objSqlhelper.ExecuteNonQuerySP("PKG_ACAD_CONVOCATION_SCHEDULER_AUTO_UPDATED_TRANSACTION", sqlparam, true);
    ...
    public int UpdateFailData(string Order_ID, int IDNO)
    {
        int retStatus1 = 0;
        int retStatus = Convert.ToInt32(CustomStatus.Others);
        try
        {
            ...
            SqlParameter[] sqlparam = null;
            {
                ...
                sqlparam[0] = new SqlParameter("@P_ORDER_ID", Order_ID);
                sqlparam[1] = new SqlParameter("@P_IDNO", IDNO);
                sqlparam[2] = new SqlParameter("@P_OUTPUT", SqlDbType.Int);
                sqlparam[2].Direction = ParameterDirection.Output;
                
            };
            object studid = objSqlhelper.ExecuteNonQuerySP("PKG_ACAD_UPDATE_FAIL_AUTO_TRANSACTION_CONVOCATION", sqlparam, true);
            return retStatus1 = Convert.ToInt32(studid);
        }
        catch (Exception ex)
        {
    ...
    public DataSet GetAllDCRTempTrasaction()
    {
        ...
        try
        {
            ...
            ds = objSqlhelper.ExecuteDataSetSP("PKG_GET_CONVOCATION_RECORD_FOR_AUTOSCHEDULAR", sqlParams);
        }
        catch (Exception ex)
        {
            return ds;
    ...
    public DataSet GetAllTransationLog()
    {
        ...
        try
        {
            ...
            ds = objSqlhelper.ExecuteDataSetSP("PKG_ACAD_GET_ONLINE_CONVOCATION_TRANSATION_LOG_REPORT", sqlParams);
        }
        catch (Exception ex)
        {
            return ds;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (dsAllTransactions != null && dsAllTransactions.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < dsAllTransactions.Tables[0].Rows.Count; i++)
            {
                ...
                ApTranID = listStrLineElements[3].ToString();         //APTRANSACTIONID  for convocation portal only

                string transid = listStrLineElements[4].ToString();      //Trnasaction id for convocation portal only

                if (status == "0300" && refundstatus == "NA")
                {
                    SuccessCounter++;
                    int resultOut = SubmitData(order_id, IDNO, "1", "CONV", TranDate, ApTranID, "1", TranMesasge, transid);
                }
                else
                {
                    FailedCounter++;
                    int resultOut = UpdateFailData(order_id, IDNO);
                }
            }
            lblSuccessStatus.Text = "Success Transaction :-" + SuccessCounter.ToString();
            lblFailedStatus.Text = "Failed Transaction :-" + FailedCounter.ToString();
