...
public partial class ACADEMIC_POSTADMISSION_ApplicationProcessNewGridview : System.Web.UI.Page
{
    ...
    protected void ddlAdmBatch_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        divgvApplicationProcess.Style["display"] = "none";
    ...
    protected void ddlDegreeProgram_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            gvApplicationProcess.UseAccessibleHeader = true;
            gvApplicationProcess.HeaderRow.TableSection = TableRowSection.TableHeader;
        }
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
    ...
    private void BindListView(int ctrlid)
    {
        try
        {
            ...
            if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                gvApplicationProcess.DataSource = dt;
                gvApplicationProcess.DataBind();
                gvApplicationProcess.UseAccessibleHeader = true;
                gvApplicationProcess.HeaderRow.TableSection = TableRowSection.TableHeader;
                divgvApplicationProcess.Style["display"] = "inline";


                if (Convert.ToInt32(Session["usertype"].ToString()) == 3)
                {
                    gvApplicationProcess.Columns[0].Visible = false;
                }
                else
                {
                    gvApplicationProcess.Columns[0].Visible = true;
                }
                if (chkstatus.ToString().Trim() == "P")
                {
                    gvApplicationProcess.Columns[4].Visible = true;
                    gvApplicationProcess.Columns[5].Visible = false;
                }
                else
                {
                    gvApplicationProcess.Columns[4].Visible = false;
                    gvApplicationProcess.Columns[5].Visible = true;
                }
                if (chkstatus.ToString().Trim() == "V")
                {
                    gvApplicationProcess.Columns[2].Visible = true;
                }
                else
                {

                    gvApplicationProcess.Columns[2].Visible = false;
                }
                //if (ctrlid == 1)
                //{
                //    divCurrentAppStage.Style["display"] = "inline";
                ...
                if (ctrlid == 2)
                {
                    if (Convert.ToInt32(ddlCurrentAppStage.SelectedIndex) == 3)
                    {
                        gvApplicationProcess.Columns[10].Visible = false;
                        gvApplicationProcess.Columns[11].Visible = false;
                    }
                    else
                    {
                        gvApplicationProcess.Columns[10].Visible = true;
                        gvApplicationProcess.Columns[11].Visible = true;
                    }
                    if (Convert.ToInt32(Session["usertype"].ToString()) == 1)
                    {
            ...
            {
                gvApplicationProcess.DataSource = null;
                gvApplicationProcess.DataBind();
                divgvApplicationProcess.Style["display"] = "none";
                //if (ctrlid == 1)
                //{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow row in gvApplicationProcess.Rows)
            {
                HiddenField hduserno = row.FindControl("hdnuserno") as HiddenField;
                CheckBox chStud = row.FindControl("chkStud") as CheckBox;
    ...
    protected void btnSendEmail_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow row in gvApplicationProcess.Rows)
            {
                CheckBox ckStud = row.FindControl("chkStud") as CheckBox;
                Label lbEmail = row.FindControl("lblEmail") as Label;
                Label lbStudName = row.FindControl("lblFullName") as Label;
                Label lbEmailSend = row.FindControl("lblEmailSend") as Label;
                HiddenField hduserno = row.FindControl("hdnuserno") as HiddenField;
                Label lbApplicationId = row.FindControl("lblApplicationId") as Label;
                Label lbMobileNo = row.FindControl("lblMobileNo") as Label;

                if (ckStud.Checked == true)
                {
                    Checked_count++;
