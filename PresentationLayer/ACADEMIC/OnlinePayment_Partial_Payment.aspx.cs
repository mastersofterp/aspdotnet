...
//PAGE NAME : ONLINE PAYMENT
//CREATION DATE : 15-01-2021
//CREATED BY : ROHIT MORE
//MODIFIED DATE : 26-02-2024
//MODIFIED DESC : Added code for provision for Online Payment
//==============================================================================
...
public partial class ACADEMIC_OnlinePayment : System.Web.UI.Page
{
    ...
    public void SemesterWiseFees()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            if (total > 0)
            {
                ...
                txtPartAmount.Text = total.ToString();
                hdfAmount.Value = total.ToString();//ds.Tables[0].Rows[0]["FEESAMOUNT"].ToString();
                hdfAmountPart.Value = total.ToString();//ds.Tables[0].Rows[0]["FEESAMOUNT"].ToString();
                Session["PayableAmount"] = txtPartAmount.Text.Trim();


                int partial_Payment = Convert.ToInt32(objCommon.LookUp("ACD_MODULE_CONFIG", "ISNULL(ENABLE_PARTIAL_PAYMENT,0)", ""));
                Double MinAmount = Convert.ToDouble(objCommon.LookUp("ACD_MODULE_CONFIG", "ISNULL(MIN_AMOUNT_PARTIAL_PAYMENT,0)", ""));
                if (partial_Payment == 1)
                {
                    txtPartAmount.Enabled = true;
                    divparnote.Visible = true;
                    divpartpay.Visible = true;
                }
                if (MinAmount == 0)
                {
                    //objCommon.DisplayMessage(this, "Configured Minimum Amount is 0.", this.Page);
                ...
                if (partial_Payment == 1 && Convert.ToDouble(input) < MinAmount)
                {
                    divparnote.Visible = true;
                    divval.Visible = true;
                    lblval.Text = "Minimum Amount is greater than Payable Amount You are applicable for full payment only.";
                    //objCommon.DisplayMessage(this, "Configured Minimum Amount is greater than Payable Amount.", this.Page);
                    txtPartAmount.Text = string.Empty;
                    txtPartAmount.Text = total.ToString();
                    txtPartAmount.Enabled = false;
                    //divparnote.Visible = false;
                ...
                //else if (Convert.ToDouble(input) > Convert.ToDouble(Session["PayableAmount"]))
                //{
                //    objCommon.DisplayMessage(this, "Payable Amount Limit Reached.", this.Page);
                //    lblAmount.Text = string.Empty;
                //    hdfAmount.Value = lblAmount.Text.Trim();
    ...
    protected void btnPayment_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (Session["Partial_Pay"] == "5151")
            {
                Session["Installmentno"] = 51;
                int Demandno = Convert.ToInt32(objCommon.LookUp("ACD_DEMAND", "TOP 1 ISNULL(DM_NO,0)", "IDNO= " + Convert.ToInt32(Session["idno"].ToString()) + " AND SEMESTERNO=" + Convert.ToInt32(ddlSemester.SelectedValue) + " AND RECIEPT_CODE = '" + Convert.ToString(ddlReceiptType.SelectedValue) + "' AND ISNULL(CAN,0)=0 AND ISNULL(DELET,0)=0"));
                Session["demandno"] = Demandno;
                amount = Convert.ToString(hdfAmountPart.Value);
            }
            else
            {
    ...
    protected void lblAmount_TextChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(input))
            {
                ...
                {
                    divval.Visible = true;
                    lblval.Text = "Please Enter Valid Amount.";
                ...
                {
                    if (Convert.ToDouble(input) < Minamount)
                    {
                        divval.Visible = true;
                    ...
                    {
                        divval.Visible = true;
                        lblval.Text = "Enter an amount less than or equal to Payable Amount";
                        //objCommon.DisplayMessage(this, "Payable Amount Limit Reached.", this.Page);
                        txtPartAmount.Text = string.Empty;
                        //txtPartAmount.Text = Session["PayableAmount"].ToString();
                        return;
                ...
                divval.Visible = false;
                hdfAmountPart.Value = txtPartAmount.Text.Trim();
                Session["Partial_Pay"] = "5151";
            }
            else
            {
                txtPartAmount.Text = Convert.ToString(Session["PayableAmount"]);
                return;
            }
        }
        catch (Exception Ex)
