...
public partial class ACADEMIC_BulkTransportHostelUpdate : System.Web.UI.Page
    {
        ...
        try
            ...
            ds = studCont.GetStudentListForTransportationStatus(Convert.ToInt32(ddlColg.SelectedValue), Convert.ToInt32(ddlAdmbatch.SelectedValue), Convert.ToInt32(ddlDegree.SelectedValue), Convert.ToInt32(ddlBranch.SelectedValue), Convert.ToInt32(ddlYear.SelectedValue), Convert.ToInt32(ddlSemester.SelectedValue));
                ...
                foreach (ListViewDataItem item in lvStudentRecords.Items)
                    {
                    HiddenField Hosteller = item.FindControl("hdfhosteller") as HiddenField;
                    ...
                    bool Hosteler = Convert.ToBoolean(Hosteller.Value);
                    if (Number != 0 || Hosteler!=false)
                        {
                    ...
                    //bool Number = Convert.ToBoolean(TRANSTATUSNO.Value);
                    //if (Number != false)
                    //    {
                    //    chkAccept.Checked = true;
                    //    }
    ...
    protected void lvStudlist_ItemDataBound(object sender, ListViewItemEventArgs e)
        ...
        try
            ...
            if ((e.Item.ItemType == ListViewItemType.DataItem))
                ...
                DropDownList ddltransportation = (DropDownList)e.Item.FindControl("ddlTransport1");
                ...
                int transportationstat = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT S LEFT JOIN ACD_TRANSPORTATION_BULK_UPDATE_STATUS TS ON S.IDNO=TS.IDNO AND S.SEMESTERNO=TS.SEMESTERNO ", "ISNULL(TS.TRANSPORTATION,0) AS STUD_HOSTEL_STATUS", "S.IDNO=" + idno));
                if (transportationstat != null)
                    {
                    ddltransportation.SelectedValue = transportationstat.ToString();
                ...
                if (Convert.ToInt32(exist1) > 0)
                    ...
                    int transportation = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT S LEFT JOIN ACD_TRANSPORTATION_BULK_UPDATE_STATUS TS ON S.IDNO=TS.IDNO AND S.SEMESTERNO=TS.SEMESTERNO ", "ISNULL(TS.TRANSPORTATION,0) AS STUD_HOSTEL_STATUS", "S.IDNO=" + idno));
                    if (transportation != null)
                        {
                       ((DropDownList)e.Item.FindControl("ddlTransport1")).SelectedValue = transportation.ToString();
                        }
