...
public partial class ACADEMIC_ONLINEFEECOLLECTION_RazorPayOnlinePaymentRequestSemester_Admission : System.Web.UI.Page
{
    #region class
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                if (Convert.ToInt32(Session["Installemt_Flag"]) == 1)
                {
                    lblamount.Text = Session["studAmt"].ToString();
                    Session["studAmt"] = lblamount.Text.ToString();

                }
                else
                {
                    Session["Installemt_Flag"] = 0;
                }


                if (Convert.ToInt32(Session["Discount_Flag"]) == 1)
                {
                    lblamount.Text = Session["Discount_Net_Amount"].ToString();
                    Session["studAmt"] = lblamount.Text.ToString();
                }
                else
                {
                ...
                {
                    lblamount.Text = Session["Scholarship_Net_Amount"].ToString();
                    Session["studAmt"] = lblamount.Text.ToString();
                }
                else
                {
    ...
    Reprocess:
        ...
        int semadmschemeno = Convert.ToInt32(Session["STUD_SCHEMENO"]);
        int sessionno = Convert.ToInt32(Session["SemadmSessionno"]);
        string Groupids = string.Empty;
        if (Convert.ToInt32(Session["HASGROUP"]) == 1)
        {
            Groupids = Session["Groupids"].ToString();
        }
        ...
        if (Convert.ToInt32(Session["Installmentno"]) > 0)
        {
            ...
            result = objFees.InsertInstallmentOnlinePayment_TempDCRnew(Convert.ToInt32(Session["idno"]), Convert.ToInt32(Session["demandno"]), Convert.ToInt32(Session["paysemester"]), refno, Convert.ToDouble(lblamount.Text), Convert.ToString(Session["ReceiptType"]), Convert.ToInt32(Session["userno"]), "-", Convert.ToInt32(Session["SemRegflag"]), Groupids, semadmschemeno, sessionno);
        }
        else if (Convert.ToInt32(Session["Discount_Flag"]) > 0)
        {
            result = objFees.InsertInstallmentOnlinePayment_TempDCRnew(Convert.ToInt32(Session["idno"]), Convert.ToInt32(Session["demandno"]), Convert.ToInt32(Session["paysemester"]), refno, Convert.ToDouble(lblamount.Text), Convert.ToString(Session["ReceiptType"]), Convert.ToInt32(Session["userno"]), "-", Convert.ToInt32(Session["SemRegflag"]), Groupids, semadmschemeno, sessionno);

        }
        else if (Convert.ToInt32(Session["Scholarship_Flag"]) > 0)
        {
            result = objFees.InsertInstallmentOnlinePayment_TempDCRnew(Convert.ToInt32(Session["idno"]), Convert.ToInt32(Session["demandno"]), Convert.ToInt32(Session["paysemester"]), refno, Convert.ToDouble(lblamount.Text), Convert.ToString(Session["ReceiptType"]), Convert.ToInt32(Session["userno"]), "-", Convert.ToInt32(Session["SemRegflag"]), Groupids, semadmschemeno, sessionno);
        }
        else
        {
            result = objFees.InsertOnlinePayment_TempDCRnew(Convert.ToInt32(Session["idno"]), Convert.ToInt32(Session["paysession"]), Convert.ToInt32(Session["paysemester"]), refno, 1, Convert.ToString(Session["ReceiptType"]), "-", Convert.ToInt32(Session["userno"]), Convert.ToInt32(Session["SemRegflag"]), Groupids, semadmschemeno, sessionno);
        }
        if (result > 0)
        {
