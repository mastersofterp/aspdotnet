...
public partial class ACADEMIC_ONLINEFEECOLLECTION_OmniwareOnlinePaymentRequest_Split : System.Web.UI.Page
{
   ...
   // DataSet ds1 = null;
    string convertSplitInfoToJson = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                lblYear.Text = Convert.ToString(Session["YEARNO"]);
                ...
                if (ds1.Tables[0] != null && ds1.Tables[0].Rows.Count > 0)
                {
                    ...
                    //---------------------ADDED BY GAURAV S 20-10-2023 ACCESS CODE AS VENDER CODE FOR MULTIPLE ACCOUNT SETTALMENT
    ...
    Reprocess:
        ...
        else if (Session["ReceiptType"].ToString() == "PRF" || Session["ReceiptType"].ToString() == "RF")
        {
        ...
        #region added for splitting vender code in json
        //List<Vendor> vendors = (from DataRow dr in ds1.Tables[0].Rows
        //                        select new Vendor
        //                        {
        //                            split_amount_percentage  = "2.00",
        //                            vendor_code = "423196VEN939472"
                                   
        //       
                // }).ToList();
        DataSet ds1 = objFees.GetOnlinePaymentConfigurationDetails_WithDegree(Convert.ToInt32(Session["OrgId"]), Convert.ToInt32(Session["paymentId"]), Convert.ToInt32(Session["payactivityno"]), Convert.ToInt32(degreeno), Convert.ToInt32(college_id));
        if (ds1.Tables[0].Rows.Count > 0)
        {
            List<Vendor> vendors = (from DataRow dr in ds1.Tables[0].Rows
                                    select new Vendor
                                    {
                                        split_amount_fixed = Session["studAmt"].ToString(),
                                        //split_amount_percentage=Session["studAmt"].ToString(),
                                        vendor_code = ViewState["accesscode"].ToString()

                                    }).ToList();
            // ViewState["accesscode"]
            Root splitInfo = new Root();
            splitInfo.vendors = vendors;
             convertSplitInfoToJson = JsonConvert.SerializeObject(splitInfo);
           // string hdnVendor = convertSplitInfoToJson.ToString();
        }
        else {

            objCommon.DisplayMessage( "Payment configuration is not done for this session.", this.Page);
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(Request.Form["hash"])) // generating hash value
            {
                ...
                foreach (string hash_var in hashVarsSeq)
                {
                    ...
                    else if (hash_var == "zip_code")
                    {
                        ...
                        //ADDED by GAURAV FOR VENDOR CODE 20_10_2023
                    else if (hash_var == "split_info")
                    {
                        hash_string = hash_string + convertSplitInfoToJson.Trim();
                        hash_string = hash_string + '|';
                    }
                    else
                    {
            ...
            if (!string.IsNullOrEmpty(hash))
            {
                ...
                data.Add("split_info", convertSplitInfoToJson.Trim());
    ...
    private string PreparePOSTForm(string url, System.Collections.Hashtable data)      // post form
    {
        ...
        strForm.Append("<form id=\"" + formID + "\" name=\"" +
                       formID + "\" action=\"" + url +
                       "\" method=\"POST\">");
        // ADDED BY GAURAV FOR SPLIT_INFO 20_10_2023
        foreach (System.Collections.DictionaryEntry key in data)
        {

            if (key.Key == "split_info")
            {
    ...
    #region ADDED BY GAURAV FOR RENDER CODE SPLITING
    public class Vendor
    {
        public string vendor_code { get; set; }
        public string split_amount_fixed { get; set; }
        public string split_amount_percentage { get; set; }
    }
    public class Root
    {
