...
public partial class OBE_AssignSubjectExamQuestionPattern : System.Web.UI.Page
{
            ...
            foreach (RepeaterItem repItem in repeater.Items) 
            {
                ...
                foreach (ListItem item in ddlCO.Items)
                {
                    ...
                    if (item.Selected == true)
                    {
                        ...
                        Codata["Weightage"] = Weightages;//0.00;
            ...
            foreach (RepeaterItem repItem in repeater.Items)
            {
                ...
                foreach (ListItem item in BloomId.Items)
                {
                    ...
                    if (item.Selected == true)
                    {
                        ...
                        Bloomdata["Weightage"] = Weightages;//0.00;
    ...
    protected void lnkCreate_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            else if ((int)ret == 0)
            {
                ...
                #region Internal ,external marks of perticular Course.

                int COURSENO = Convert.ToInt32(objCommon.LookUp("tblacdschemesubjectmapping", "SUBJECTID", "SchemeSubjectId=" + schemesubjectid));
                string Internalmark = Convert.ToString(objCommon.LookUp("ACD_COURSE", "MAXMARKS_I", "COURSENO=" + COURSENO));
                Internalmark = Internalmark == string.Empty ? "0" : Internalmark;
                Session["Internalmark"] = Internalmark;
                string ExternalMark = Convert.ToString(objCommon.LookUp("ACD_COURSE", "MAXMARKS_E", "COURSENO=" + COURSENO));
                ExternalMark = ExternalMark == string.Empty ? "0" : ExternalMark;
                Session["ExternalMark"] = ExternalMark;
                Decimal Total = Convert.ToDecimal(Internalmark) + Convert.ToDecimal(ExternalMark);
                Session["Total"] = Total;

                txtinternal.Text = Session["Internalmark"].ToString();
                txtExternal.Text = Session["ExternalMark"].ToString();
                txttot.Text = Session["Total"].ToString();
    ...
    protected void txtMaximumMarks_TextChanged(object sender, EventArgs e)
    {
       objCommon.FillDropDownList(ddlPattern, "tblQuestionPatternMaster", "QuestionPatternId", "QuestionPatternName", "MARKS=" + txtMaximumMarks.Text + "", "QuestionPatternId");
       dvQuestionPaper.Visible = false;
       dvSubmitData.Visible = false;
