...
public partial class ESTABLISHMENT_LEAVES_Reports_LeaveTypeReport : System.Web.UI.Page
{
    string date = "";
    int counter = 0;
    ...
    private void FillDepartment()
    {
        try
        {
            ...
            {
                objCommon.FillDropDownList(ddldept, "PAYROLL_SUBDEPT", "SUBDEPTNO", "SUBDEPT", "SUBDEPTNO in (SELECT DISTINCT (SUBDEPTNO) FROM PAYROLL_EMPMAS WHERE COLLEGE_NO=" + ddlCollege.SelectedValue + ")AND  SUBDEPTNO <> 0", "SUBDEPT");
            }
            else
            {
    ...
    private void FillStaffType()
    {
        try
        {
            if (ddlCollege.SelectedValue != "0")
            {
                objCommon.FillDropDownList(ddlstafftype, "PAYROLL_STAFFTYPE", "STNO", "STAFFTYPE", "STNO in (SELECT DISTINCT (STNO) FROM PAYROLL_EMPMAS WHERE COLLEGE_NO=" + ddlCollege.SelectedValue + ")AND STNO>0 AND ISNULL(ACTIVESTATUS,0) =" + 1, "STAFFTYPE");
            }
            else
            {
