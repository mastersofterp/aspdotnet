...
// PAGE NAME     : ShiftTimereport.aspx                                    
...
public partial class ShiftTime_Report: System.Web.UI.Page
{
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //SELECT * from PAYROLL_LEAVE_ASSIGNSHIFT_DAYWISE where dt between '2015-07-21' AND '2015-08-20' 
               // int ret = Convert.ToInt32(objLC.CheckAttendance(Convert.ToDateTime(txtFromdt.Text), Convert.ToDateTime(txtTodt.Text)));
            DataSet ds = objCommon.FillDropDown("PAYROLL_LEAVE_ASSIGNSHIFT_DAYWISE", "*", "", "DT BETWEEN '" + dt1.ToString("yyyy-MM-dd") + "' AND '" + dt2.ToString("yyyy-MM-dd") + "' ", "");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ShowReport("ShiftTime", "ESTB_ShiftTimeReport.rpt");
                }
                else
                {
    ...
    private void ShowReport(string reportTitle, string rptFileName)
    {
        try
        {
           ...
           // url += "&param=@P_COLLEGE_CODE=" + Session["colcode"].ToString() + ",@P_FSDATE=" + Convert.ToDateTime(txtFromdt.Text.Trim()).ToString("yyyy-MM-dd") + ",@P_FEDATE=" + Convert.ToDateTime(txtTodt.Text.Trim()).ToString("yyyy-MM-dd");
            url += "&param=@P_FSDATE=" + Convert.ToDateTime(txtFromdt.Text.Trim()).ToString("yyyy-MM-dd") + ",@P_FEDATE=" + Convert.ToDateTime(txtTodt.Text.Trim()).ToString("yyyy-MM-dd") + ",@P_COLLEGE_CODE=" + Session["colcode"].ToString();
