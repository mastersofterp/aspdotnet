...
public partial class ACCOUNT_AccVoucherVerifyApprove : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                if (Request.QueryString["Action"] == "VerifyBack")
                {
                    ShowMessage("Record Verified Successfully.");
                    showData();
                }
                if (Request.QueryString["Action"] == "ApproveBack")
                {
    ...
    public void showData()
    {
        ...
        {
            objCommon.DisplayMessage(this.Page, "Upto Date Should Be In The Financial Year Range. ", this);
        ...
        {
            objCommon.DisplayMessage(this.Page, "From Date Should Be In The Financial Year Range. ", this);
        ...
        {
            objCommon.DisplayMessage(this.Page, "From Date Can Not Be Greater Than Upto Date Date. ", this);
            txtUptoDate.Focus();
            return;
        }
    ...
    private void AddGridEntry()
    {
        ...
        if (PartyId != null)
        {
            ...
            {
                objCommon.DisplayMessage(this.Page, "Invalid Ledger No.", this);
                txtAcc.Focus();
                return;
            }
        ...
        if (ds != null)
        {
            ...
            if (dsAuth.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    dsvch = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS", "DISTINCT VOUCHER_NO as VOUCHER_NO,VOUCHER_SQN,TRANSACTION_DATE,(CASE PAYMENT_MODE WHEN 'C' THEN 'CHEQUE'  WHEN 'N' THEN 'NEFT' WHEN 'R' THEN 'RTGS' WHEN 'B' THEN 'BANK' WHEN 'O' THEN 'ONLINE' ELSE '' END) AS PAYMENT_MODE", "Transaction_Type,CHQ_NO", "transaction_type <> 'OB' and ISVerified IS NULL and transaction_date between '" + Convert.ToDateTime(txtFrmDate.Text).ToString("dd-MMM-yyyy") + "' and '" + Convert.ToDateTime(txtUptoDate.Text).ToString("dd-MMM-yyyy") + "'", "TRANSACTION_DATE,Transaction_Type,VOUCHER_NO");//party_no=" + id.ToString().Trim() + " and 
                ...
                {
                    dsvch = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS", "DISTINCT VOUCHER_NO as VOUCHER_NO,VOUCHER_SQN,TRANSACTION_DATE,(CASE PAYMENT_MODE WHEN 'C' THEN 'CHEQUE' WHEN 'N' THEN 'NEFT' WHEN 'R' THEN 'RTGS'  WHEN 'B' THEN 'BANK' WHEN 'O' THEN 'ONLINE' ELSE '' END) AS PAYMENT_MODE", "Transaction_Type,CHQ_NO", "transaction_type <> 'OB' and ISVerified=1 and ISApproved IS NULL and transaction_date between '" + Convert.ToDateTime(txtFrmDate.Text).ToString("dd-MMM-yyyy") + "' and '" + Convert.ToDateTime(txtUptoDate.Text).ToString("dd-MMM-yyyy") + "'", "TRANSACTION_DATE,Transaction_Type,VOUCHER_NO");//party_no=" + id.ToString().Trim() + " and 
            ...
            dsOtran = objCommon.FillDropDown("(SELECT * FROM ACC_ALL_TEMP_TRANS O WHERE O.TRANSACTION_TYPE+'_'+O.VOUCHER_NO IN (SELECT DISTINCT I.TRANSACTION_TYPE+'_'+I.VOUCHER_NO FROM ACC_ALL_TEMP_TRANS  I)) a inner join ACC_" + Session["comp_code"].ToString() + "_PARTY AP on(a.PARTY_NO=ap.PARTY_NO)", "CONVERT(VARCHAR(8), a.TRANSACTION_DATE, 3) as Date,AP.PARTY_NAME as Particulars,CASE A.TRANSACTION_TYPE WHEN 'P'  THEN 'PAYMENT' WHEN  'R' THEN 'RECEIPT' WHEN 'C' THEN 'CONTRA' ELSE 'JOURNAL' END AS VchType,a.VOUCHER_NO as VchNo", "(case  A.[TRAN]	when  'Dr' then A.amount else 0 end) as CREDIT,(case  A.[TRAN]	when  'Cr' then A.amount else 0 end) as DEBIT", "a.transaction_date between '" + Convert.ToDateTime(txtFrmDate.Text).ToString("dd-MMM-yyyy") + "' and '" + Convert.ToDateTime(txtUptoDate.Text).ToString("dd-MMM-yyyy") + "'", "A.TRANSACTION_NO,A.VOUCHER_NO");// WHERE I.PARTY_NO = '" + partyNo + "'
            dt = dsOtran.Tables[0];
            Session["DatatableMod"] = dt;
            dsOtran.Dispose();
        }
    ...
    protected void RptData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        if (lbvch != null)
        {
            ...
            else if (e.CommandName.ToString().Trim() == "VoucherDelete")
            {
                if (hdnAskDelete.Value == "1")
                {
                    ...
                    if (Convert.ToInt32(TF_ENTRY.Tables[0].Rows[0][0].ToString()) == 0)
                    {
                        ...
                        {
                            objCommon.DisplayUserMessage(this.Page, "Voucher Deleted successfully.", this);
                            showData();
                    ...
                    {
                        objCommon.DisplayUserMessage(this.Page, "Transferd RptDataEntry Can Not Be Deleted .", this);
                        showData();
                    }
                }
            }
            ...
            else if (e.CommandName.ToString().Trim() == "CopyVoucher")
            {
                ...
                {
                    objCommon.DisplayUserMessage(this.Page, "Transferd Entry Can Not Be Copied.", this);
                    showData();
                }

            }
            else if (e.CommandName.ToString().Trim() == "VoucherDetails")
            {
                string FinancialDate = objCommon.LookUp("ACC_COMPANY", "cast(COMPANY_FINDATE_FROM as nvarchar(20))" + "+ ''' and ''' +" + "cast(COMPANY_FINDATE_TO as nvarchar(20))", "COMPANY_CODE='" + Session["comp_code"].ToString() + "'");
                ...
                if (dsEmployeeData != null && dsEmployeeData.Tables[0].Rows.Count > 0)
                {
                    ...
                    Response.Redirect("BulkVouchers.aspx?obj=configm," + cmd.ToString().Trim() + "," + lbvch.Text.ToString().Trim() + "," + txtFrmDate.Text.ToString().Trim() + "," + txtUptoDate.Text.ToString().Trim() + "," + ViewState["AuthorityType"].ToString() + "&pageno=" + Request.QueryString["pageno"]);
                }
                else
                {
                    objCommon.DisplayUserMessage(this.Page, "No Employees Found", this);
                    showData();
                }
            }
            else if (e.CommandName.ToString().Trim() == "ViewBill")
            {
                DataSet dsEmployeeData = objCommon.FillDropDown("ACC_TEMP_VOUCHER_UPLOAD_DOCUMENT", "*", "", "VOUCHER_SQN=" + cmd.ToString().Trim(), "");
                if (dsEmployeeData.Tables[0].Rows.Count == 0)
                {
                    objCommon.DisplayUserMessage(this.Page, "Bill Is Not Found", this);
                }
                else
                {
                    if (dsEmployeeData.Tables[0].Rows.Count > 0)
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        objCommon.DisplayUserMessage(this.Page, "Transferd Entry Can Not Be Modified .", this);
                        showData();
                    }
                }
            }
    ...
    protected void btnShowTrialBalance_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                objCommon.DisplayMessage(this.Page, "Enter From Date", this);
                txtFrmDate.Focus();
                return;
            }
            if (txtUptoDate.Text.ToString().Trim() == "")
            {
                objCommon.DisplayMessage(this.Page, "Enter Upto Date", this);
                txtUptoDate.Focus();
                return;
            }
    ...
    private void ShowDayBook(string reportTitle, string rptFileName)
    {
        try
        {
            ...
            //ScriptManager.RegisterClientScriptBlock(this.this.Page, this.Page.GetType(), "Report", Script, true);
    ...
    protected void RptData_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            HiddenField hdnVoucherSQNNo = e.Item.FindControl("hdnVoucherSQNNo") as HiddenField;
            ...
            int UserId = Convert.ToInt32(objCommon.LookUp("ACC_ALL_TEMP_TRANS", "[USER]", "VOUCHER_NO=" + hdnVoucherNo.Value + " AND VOUCHER_SQN=" + hdnVoucherSQNNo.Value));
            string USER_NAME = objCommon.LookUp("USER_ACC", "UA_FULLNAME", "UA_NO=" + UserId);
         
            Label lblcreatedby = (Label)e.Item.FindControl("lblcreatedby");
            lblcreatedby.Text = USER_NAME;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < RptData.Items.Count; i++)
        {
            ...
            //added by tanu 26/02/2022
            HiddenField hdvocherno = RptData.Items[i].FindControl("hdnVoucherNo") as HiddenField;
            Label lbpaytype = RptData.Items[i].FindControl("lblvchtype") as Label;
            string VCH_TYPE = string.Empty;

            if (lbpaytype.Text == "Payment")
                VCH_TYPE = "P";
            else if (lbpaytype.Text == "Receipt")
                VCH_TYPE = "R";
            else if (lbpaytype.Text == "Contra")
                VCH_TYPE = "C"; 
            else if (lbpaytype.Text == "Journal")
                VCH_TYPE = "J";
            if (chkRow.Checked)
            {
                //added by tanu 26/02/2022
              
                DataSet ds = objCommon.FillDropDown("ACC_" + Session["comp_code"].ToString() + "_TRANS", "*", "", "TEMP_VOUCHER_NO=" + Convert.ToInt32(hdvocherno.Value) +" and TRANSACTION_TYPE='" + VCH_TYPE+ "'", "");
                  if (ds.Tables[0].Rows.Count>0)
                  {
                      objCommon.DisplayMessage("Voucher Number  " +Convert.ToInt32(chkRow.ToolTip)+"  Alredy Approved .", this);
                      return;
                  }
        ...
        try
        {
            if (PATH != null && PATH != string.Empty && PATH != "")
            {
                ...
                if (System.IO.Directory.Exists(PATH))
                {
                    ...
                    {
                        lvNewBills.DataSource = files;
                        lvNewBills.DataBind();
                      
                        lvNewBills.Visible = true;
                    }
                    else
                    {
                        lvNewBills.DataSource = null;
