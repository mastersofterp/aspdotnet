...
public partial class ACCOUNT_AdvanceVoucher : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            ViewState["TDSonGST"] = string.Empty;
            ViewState["IsIGST"] = "0";
            ViewState["IsGST"] = "0";
            ...
            objCommon.FillDropDownList(ddlEmployee, "PAYROLL_EMPMAS E INNER JOIN PAYROLL_PAYMAS P ON (E.IDNO = P.IDNO) INNER JOIN PAYROLL_SUBDEPT D on( E.SUBDEPTNO = D.SUBDEPTNO)", "E.IDNO", "isnull(E.FNAME,'')+' '+isnull(E.MNAME,'')+' '+isnull(E.LNAME,'')+' - '+ISNULL(D.SUBDEPT,'') AS NAME", "PSTATUS='Y'", "  E.FNAME");
    ...
    public static string LookupProduct(string Username)
    {
        ...
        //try
        //{
        var thisPage = new ACCOUNT_AdvanceVoucher();
    ...
    private void UpdateDataRow_Journal(int Index, DataTable dtupd)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            if (Convert.ToDouble(dt.Rows[i]["IGSTper"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["SGSTper"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["CGSTper"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["TDSpercentage"].ToString()) == 0.00 && dt.Rows[i]["Mode"].ToString().Trim() == "Dr")
            {
                ...
                dt.Rows[i]["TDSonGSTAMOUNT"] = "0.00";
                dt.Rows[i]["TDSonGSTsection"] = "0";
                dt.Rows[i]["TDSonGSTpercentage"] = "0.00";
            ...
            if (Convert.ToDouble(dt.Rows[i]["TDSpercentage"].ToString()) > 0)
            {
                ...
                // added by Shrikant A on 21-04-2021
                dt.Rows[i]["TDSonGSTAMOUNT"] = String.Format("{0:0.00}", Math.Abs(Convert.ToDouble(txtTDSGSTonAmount.Text.ToString().Trim()))); ;
                dt.Rows[i]["TDSonGSTsection"] = ddlTDSonGSTSection.SelectedValue;
                dt.Rows[i]["TDSonGSTpercentage"] = Convert.ToDouble(txtTDSonGSTPer.Text).ToString();
    ...
    private void AddGridEntry()
    {
        ...
        // added by Shrikant A on 21-04-2021
        row["TDSonGSTAMOUNT"] = "0";
        row["TDSonGSTsection"] = "0";
        row["TDSonGSTpercentage"] = "0";
        row["EmployeeName"] = ddlEmployee.SelectedItem.Text.ToString().Trim().Split('-')[0];
        row["EmployeeId"] = ddlEmployee.SelectedValue;
        ...
        {
            if (txtTDSonGSTAmount.Text != "" || txtTDSonGSTAmount.Text != string.Empty || txtTDSonGSTPer.Text != string.Empty || txtTDSonGSTPer.Text != "")
            {
                AddTDSonGSTGrid();
            }
        }
    ...
    protected void UpdateRowNew()
    {
        ...
        row["TDSonGSTAMOUNT"] = "0.00";
        row["TDSonGSTsection"] = "0";
        row["TDSonGSTpercentage"] = "0.00";
        dt.Rows.Add(row);
        ...
        if (ViewState["TDSonGST"].ToString() == "Yes")
        {
            ...
            row = dt.NewRow();
            row["Particulars"] = txtTDSonGSTLedger.Text.ToString().Trim();
            row["Narration"] = txtPerNarration.Text.ToString().Trim().Replace("¯", "");
            ...
            partyNo = objCommon.LookUp("ACC_" + Session["comp_code"].ToString() + "_PARTY", "PARTY_NO", "ACC_CODE='" + txtTDSonGSTLedger.Text.ToString().Trim().Split('*')[1] + "'");
            count = 0;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (dt.Rows[i]["Id"].ToString() == partyNo)
            ...
            if (dtr.Read())
            {
                string tdsongstAmt = dtr["BALANCE"].ToString().Trim();
                ViewState["Balance3"] = tdsongstAmt.ToString();// dtr["BALANCE"].ToString().Trim();
            }
            if (count > 0)
            {
                string balance = string.Empty;
            ...
            if (TranMode.ToString().Trim() == "Dr")
            {
                if (ViewState["Balance3"].ToString().Trim() != "")
                {
                    row["Balance"] = (Convert.ToDouble(ViewState["Balance3"].ToString().Trim()) + Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text.ToString().Trim()))).ToString().Trim();
                    ViewState["Balance3"] = row["Balance"].ToString();
                }
                else
                {
            ...
            else
            {
                if (ViewState["Balance3"].ToString().Trim() != "")
                {
                    row["Balance"] = (Convert.ToDouble(ViewState["Balance3"].ToString().Trim()) - Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text))).ToString().Trim();
                    ViewState["Balance3"] = row["Balance"].ToString();
                }
                else
                {
            ...
            if (isSingleMode == "Y")
            {
                ...
                row["Debit"] = "0.00";
                row["Credit"] = "0.00";
                row["Amount"] = String.Format("{0:0.00}", Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text.ToString().Trim())));         // "123.00"    txtTranAmt.Text.ToString().Trim();
            }
            else
            {
                GridData.Columns[4].Visible = false;
                ...
                if (TranMode.ToString().Trim() == "Dr")
                {
                    row["Debit"] = String.Format("{0:0.00}", Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text.ToString().Trim())));
                    row["Credit"] = "0.00";
                }
                else
                {
                    row["Debit"] = "0.00";
                    row["Credit"] = String.Format("{0:0.00}", Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text.ToString().Trim())));
                }
            ...
            row["TDSonGSTAMOUNT"] = txtTDSGSTonAmount.Text;
            row["TDSonGSTsection"] = ddlTDSonGSTSection.SelectedValue;
            row["TDSonGSTpercentage"] = txtTDSonGSTPer.Text;
    ...
    protected void AddDeleteId()
    {
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            //if ((Convert.ToDouble(dt.Rows[i]["TDSonGSTpercentage"].ToString()) > 0))
            //{
            //    if (ViewState["TDSonGST"].ToString() == "Yes")
            //    {
            //        dt.Rows[i]["Dld_id"] = "4";
            //    }
            //}
            if (Convert.ToDouble(dt.Rows[i]["IGSTper"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["SGSTper"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["CGSTper"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["TDSpercentage"].ToString()) == 0.00 && Convert.ToDouble(dt.Rows[i]["TDSonGSTpercentage"].ToString()) == 0.00)
            {
    ...
    private void SetDataColumn()
    {
        ...
        else
        {
            ...
            if (dt == null)
            {
                ...
                // added by Shrikant A on 20-04-2021

                DataColumn dc30 = new DataColumn();
                dc30.ColumnName = "TDSonGSTsection";
                dt.Columns.Add(dc30);

                DataColumn dc31 = new DataColumn();
                dc31.ColumnName = "TDSonGSTAMOUNT";
                dt.Columns.Add(dc31);

                DataColumn dc32 = new DataColumn();
                dc32.ColumnName = "TDSGSTonAmount";
                dt.Columns.Add(dc32);

                DataColumn dc33 = new DataColumn();
                dc33.ColumnName = "TDSonGSTpercentage";
                dt.Columns.Add(dc33);

                DataColumn dc34 = new DataColumn();
                dc34.ColumnName = "EmployeeName";
                dt.Columns.Add(dc34);

                DataColumn dc35 = new DataColumn();
                dc35.ColumnName = "EmployeeId";
                dt.Columns.Add(dc35);
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (chkTDSonGST.Checked == true)
                {
                    if (txtTDSonGSTLedger.Text == string.Empty || txtTDSonGSTLedger.Text == "")
                    {
                        objCommon.DisplayMessage(UPDLedger, "Please Enter TDS on GST Ledger", this);
                        txtTDSLedger.Focus();
                        return;
                    }
                    if (txtTDSonGSTPer.Text == string.Empty || txtTDSonGSTPer.Text == "")
                    {
                        objCommon.DisplayUserMessage(UPDLedger, "Please Enter TDS on GST Percentage", this);
                        txtTDSPer.Focus();
                        return;
                    }

                    if (txtTDSonGSTAmount.Text == string.Empty || txtTDSonGSTAmount.Text == "")
                    {
    ...
    protected void GridData_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        ...
        if (ViewState["TDS"].ToString() == "Yes")
        {
            ...
            if (isSingleMode == "Y")
            {
                DataRow[] TDSROW = dt.Select("Particulars like '%TDS%' and TDSpercentage > '0'");
                if (TDSROW.Length > 0)
                {
        ...
        if (ViewState["TDSonGST"].ToString() == "Yes")
        {
            ...
            double TDSGSTamount = 0, TDSonGSTamount = 0;
            double TGAmount = 0; double TDSonGSTPer = 0;
            int TDSonGSTsection = 0;
            if (isSingleMode == "Y")
            {
                DataRow[] TDSROW = dt.Select("Particulars like '%TDS%' and TDSonGSTpercentage > '0'");
                if (TDSROW.Length > 0)
                {
                    txtTDSLedger.Text = TDSROW[0]["Particulars"].ToString();
                    TDSGSTamount = Convert.ToDouble(TDSROW[0]["TDSonGSTAMOUNT"].ToString());
                    TDSonGSTamount = Convert.ToDouble(TDSROW[0]["Amount"].ToString());
                    TDSonGSTPer = Convert.ToDouble(TDSROW[0]["TDSonGSTpercentage"].ToString());
                    TDSonGSTsection = Convert.ToInt32(TDSROW[0]["TDSonGSTsection"].ToString());
            ...
            else
            {
                ...
                if (ddlcrdr.SelectedValue.ToString().Trim() == "Cr")
                {
                    TGAmount = Convert.ToDouble(GridData.Rows[e.NewSelectedIndex].Cells[6].Text);
                }
                else { TGAmount = Convert.ToDouble(GridData.Rows[e.NewSelectedIndex].Cells[5].Text); }

            }


            txtTDSPer.Text = TDSonGSTPer.ToString();
            txtTDSAmount.Text = TDSonGSTamount.ToString();
            txtTamount.Text = TDSGSTamount.ToString();
            ddlSection.SelectedValue = TDSonGSTsection.ToString();
            dvTDSonGST.Visible = true;
        }
        ...
        try
        {
            ...
            if (hdnAskSave.Value == "1")
            {
                ...
                for (int i = 0; i < GridData.Rows.Count; i++)
                {
                    ...
                    XmlElement EmployeeId = objXMLDoc.CreateElement("EmployeeId");
                    HiddenField hdnEmployeeId = GridData.Rows[i].FindControl("hdnEmployeeId") as HiddenField;
                    EmployeeId.InnerText = hdnEmployeeId.Value;
                    ...
                    objElement.AppendChild(EmployeeId);
                ...
                if (objCommon.LookUp("ACC_" + Session["comp_code"] + "_CONFIG", "PARAMETER", "CONFIGDESC='AUTOGENERATED VOUCHER NO. REQUIRED'") == "Y")
                {
                    ...
                    if (isTempVoucher == "Y")
                    {
                        voucherno = objPC1.AddTransactionWithXMLMakautAdvance(objXMLDoc, Session["comp_code"].ToString().Trim(), IsModify, VoucherSqn, Session["fin_yr"].ToString().Trim(), ddlTranType.SelectedValue.ToString());
                    }
                    else
                    {
        ...
        for (int j = 0; j < 2; j++)
        {
            if (j == 0 && chkTDSonGST.Checked == false)
            {
                ...
                if (TranMode.ToString().Trim() == "Dr")
                {
                    if (ViewState["Balance2"].ToString().Trim() != "")
                    {
                        row["Balance"] = (Convert.ToDouble(ViewState["Balance2"].ToString().Trim()) + Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text.ToString().Trim()))).ToString().Trim();
                        ViewState["Balance2"] = row["Balance"].ToString();
                    }
                    else
                    {
                ...
                else
                {
                    if (ViewState["Balance2"].ToString().Trim() != "")
                    {
                        row["Balance"] = (Convert.ToDouble(ViewState["Balance2"].ToString().Trim()) - Math.Abs(Convert.ToDouble(txtTDSonGSTAmount.Text))).ToString().Trim();
                        ViewState["Balance2"] = row["Balance"].ToString();
                    }
                    else
                    {
                ...
                //row["Id"] = hdnIdEditParty.Value;
                row["Id"] = partyNo;
                row["OppParty"] = Convert.ToInt32(txtTDSonGSTLedger.Text.Trim().ToString().Split('*')[1]).ToString();
                if (isPerNarration == "Y")
                {
                    row["ChqNo"] = txtChqNo1.Text.ToString().Trim();
                    row["ChqDate"] = txtChequeDt1.Text.ToString().Trim();
            ...
            else if (j == 1)
            {
                ...
                string partyNo = objCommon.LookUp("ACC_" + Session["comp_code"].ToString() + "_PARTY", "PARTY_NO", "ACC_CODE='" + txtTDSonGSTLedger.Text.ToString().Trim().Split('*')[1] + "'");
                int count = 0;
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["Id"].ToString() == partyNo)
    ...
    protected void clearTax()
    {
        ...
        txtIGSTAMT.Text = txtIGSTPER.Text =
        txtTDSonGSTPer.Text = txtTDSGSTonAmount.Text = string.Empty;
    ...
    {
        if (txtAcc.Text == txtTDSonGSTLedger.Text)
        {
    ...
    {
        txtTDSonGSTPer.Text = objCommon.LookUp("ACC_" + Session["comp_code"].ToString() + "_SECTION", "SECTION_PERCENT", "SECTION_NO=" + ddlTDSonGSTSection.SelectedValue);
        decimal TDSOnAmount = Convert.ToDecimal(txtTDSGSTonAmount.Text) / 100;
        decimal Per = (txtTDSonGSTPer.Text == string.Empty ? Convert.ToDecimal("0") : Convert.ToDecimal(txtTDSonGSTPer.Text));
        //Convert.ToDouble(txtTranAmt.Text == string.Empty ? "0" : txtTranAmt.Text).ToString();
        decimal Total = TDSOnAmount * Per;
        txtTDSonGSTAmount.Text = Total.ToString();
    ...
    protected void chkTDSonGST_CheckedChanged(object sender, EventArgs e)
    {
        if (chkTDSonGST.Checked == true)
        {
            dvTDSonGST.Visible = true;
            txtTDSonGSTLedger.Focus();
            txtTDSGSTonAmount.Text = Convert.ToDouble(txtTranAmt.Text == string.Empty ? "0" : txtTranAmt.Text).ToString();
            objCommon.FillDropDownList(ddlTDSonGSTSection, "acc_" + Session["comp_code"].ToString() + "_section", "SECTION_NO", "SECTION_NAME", "", "");
            ViewState["TDSonGST"] = "Yes";
