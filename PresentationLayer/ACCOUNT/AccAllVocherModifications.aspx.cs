...
public partial class ACCOUNT_AccAllVocherModifications : System.Web.UI.Page
{
        ...
        if (!Page.IsPostBack)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    ViewState["DOCS"] = null;
                    DeleteDirecPath(Docpath + "VOUCHER_BILL\\EMPID_" + Convert.ToInt32(Session["userno"]) + "\\BillNo" + Convert.ToInt32(ViewState["OLD_VOUCHER_NO"]));


                }
                ViewState["NEW_VOUCHER_SQN"] = null;
            }
            SetFinancialYear();
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        ...
        {
            if (rdblistbill.SelectedValue == "A")
            {
               
                showApprovedData();
            }
            else if (rdblistbill.SelectedValue == "P")
            {
                ViewState["Approved"] = "P";
                showData();
            }
    ...
    private void AddGridEntry()
    {
        ...
        DataSet ds = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS", "*", "", "party_no=" + id.ToString().Trim() + " and transaction_type <> 'OB' and (isnull(ISApproved,0)=" + 0 + " and isnull(ISverified,0)=" + 0 + ")   and CAST(transaction_date AS DATE ) between '" + Fdate + "' and '" + Ldate + "'", "TRANSACTION_DATE,TRANSACTION_TYPE,VOUCHER_NO");
        if (ds == null)
        {
        ...
        if (ds != null)
        {
            DataSet dsvch = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS", "DISTINCT VOUCHER_NO as VOUCHER_NO,VOUCHER_SQN,TRANSACTION_DATE", "Transaction_Type,CHQ_NO,0 as TEMP_VOUCHER_NO", "party_no=" + id.ToString().Trim() + " and transaction_type <> 'OB' and (isnull(ISApproved,0)=" + 0 + " or isnull(ISverified,0)=" + 0 + ")  and CAST(transaction_date AS DATE) between '" + Fdate + "' and '" + Ldate + "'", "TRANSACTION_DATE,Transaction_Type,VOUCHER_NO");
            ...
            dsOtran = objCommon.FillDropDown("(SELECT * FROM ACC_ALL_TEMP_TRANS O WHERE O.TRANSACTION_TYPE+'_'+O.VOUCHER_NO IN (SELECT DISTINCT I.TRANSACTION_TYPE+'_'+I.VOUCHER_NO FROM ACC_ALL_TEMP_TRANS  I WHERE I.PARTY_NO = '" + partyNo + "' and  isnull(I.ISApproved,0)=" + 0 + " or isnull(I.ISverified,0)=" + 0 + ")) a inner join ACC_" + Session["comp_code"].ToString() + "_PARTY AP on(a.PARTY_NO=ap.PARTY_NO)", "CONVERT(VARCHAR(8), a.TRANSACTION_DATE, 3) as Date,AP.PARTY_NAME as Particulars,CASE A.TRANSACTION_TYPE WHEN 'P'  THEN 'PAYMENT' WHEN  'R' THEN 'RECEIPT' WHEN 'C' THEN 'CONTRA' ELSE 'JOURNAL' END AS VchType,a.VOUCHER_NO as VchNo", "(case  A.[TRAN]	when  'Dr' then A.amount else 0 end) as DEBIT ,(case  A.[TRAN]	when  'Cr' then A.amount else 0 end) as CREDIT", " (isnull(a.ISApproved,0)=" + 0 + " or isnull(a.ISverified,0)=" + 0 + ") and CAST(a.transaction_date AS DATE) between '" + Fdate + "' and '" + Ldate + "'", "A.TRANSACTION_NO,A.VOUCHER_NO");
            dt = dsOtran.Tables[0];
            Session["DatatableMod"] = dt;
            dsOtran.Dispose();
        }
    ...
    // this code for voucher modification 
        ...
        if (lbvch != null)
        {
                ...
                if (rdblistbill.SelectedValue == "A")
                {
                    ...
                    if (isFourSign == "N")
                    {
                        if (lbvch.Text.ToString().Trim() == "Payment")
                        {
                            ...
                            ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucher1.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                            //ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherRptMakaut.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);

                        }
                        if (lbvch.Text.ToString().Trim() == "Receipt")
                        {

                            ShowVoucherCashBankReport("Voucher", "PmtReceiptCashVoucher1.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                            //ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherRptMakaut.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                ...
                else
                {
                    ...
                    if (isFourSign == "N")
                    {
                        if (lbvch.Text.ToString().Trim() == "Payment" )
                        {
                            ...
                            ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherPending.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                            //ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherRptMakaut.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                        ...
                        if (lbvch.Text.ToString().Trim() == "Receipt")
                        {
                            ...
                            ShowVoucherCashBankReport("Voucher", "PmtReceiptCashVoucherPending.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                            //ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherRptMakaut.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);

                        }
                        else
                        {
                            ShowVoucherPrintReport("Voucher", "JvContraVoucherReport_Pending.rpt", lbvch.Text.ToString().Trim(), cmd);
                            //ShowVoucherPrintReport("Voucher", "JvContraVoucherReportMakaut.rpt", lbvch.Text.ToString().Trim(), cmd);
                        }
                    }
                    else if (isFourSign == "Y")
                    {
                        ...
                        {
                            //ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucher1.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                            ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherRpt_Format2_Pending.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                        }
                        else
                        {
                            ShowVoucherPrintReport("Voucher", "JvContraVoucherReport_Format2_Pending.rpt", lbvch.Text.ToString().Trim(), cmd);
                        }
                    }
                }
            ...
            else if (e.CommandName.ToString().Trim() == "VoucherDelete")
            {
                if (hdnAskDelete.Value == "1")
                {
                    ...
                    string FinancialDate = objCommon.LookUp("ACC_COMPANY", "cast(COMPANY_FINDATE_FROM as nvarchar(20))" + "+ ''' and ''' +" + "cast(COMPANY_FINDATE_TO as nvarchar(20))", "COMPANY_CODE='" + Session["comp_code"].ToString() + "'");
                    DataSet TF_ENTRY = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS", "TRANSFER_ENTRY", "", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_SQN=" + cmd.ToString().Trim(), string.Empty);
                    if (Convert.ToInt32(TF_ENTRY.Tables[0].Rows[0][0].ToString()) == 0)
                    {
                        ...
                        CustomStatus cs = (CustomStatus)objAVC.DeleteVoucherMakaut(cmd, Session["comp_code"].ToString(), VCH_TYPE);
                    ...
                    {
                        objCommon.DisplayUserMessage(UPDLedger, "Transferd Entry Can Not Be Deleted .", this);
                        showData();
                    }
                }
            }
            ...
            else if (e.CommandName.ToString().Trim() == "ViewAddBill")
            {

                DataSet dsEmployeeData = objCommon.FillDropDown("ACC_TEMP_VOUCHER_UPLOAD_DOCUMENT", "*", "", "NEW_VOUCHER_NO=" + cmd.ToString().Trim(), "");
                if (dsEmployeeData.Tables[0].Rows.Count > 0)
                {
                 
                    ViewState["NEW_VOUCHER_SQN"] = cmd.ToString();
                    Panel1.Visible = false;
                    pnlupload.Visible = true;

                    int rowCount = dsEmployeeData.Tables[0].Rows.Count;
                    CreateTable();
                    DataTable dtM = (DataTable)ViewState["DOCS"];
                    for (int i = 0; i < rowCount; i++)
                    {
                        DataRow dr = dtM.NewRow();
                        ViewState["OLD_VOUCHER_NO"] = dsEmployeeData.Tables[0].Rows[i]["VOUCHER_NO"].ToString();
                        dr["FUID"] = dsEmployeeData.Tables[0].Rows[i]["DOCID"].ToString();
                        dr["Filepath"] = dsEmployeeData.Tables[0].Rows[i]["Filepath"].ToString();
                        dr["DisplayFileName"] = dsEmployeeData.Tables[0].Rows[i]["DisplayFileName"].ToString();
                        dr["DocumentName"] = dsEmployeeData.Tables[0].Rows[i]["DocumentName"].ToString();
                        dtM.Rows.Add(dr);
                        dtM.AcceptChanges();
                        ViewState["DOCS"] = dtM;
                        ViewState["FUID"] = dsEmployeeData.Tables[0].Rows[i]["DOCID"].ToString();
                    }

                    lvNewBills.DataSource = dsEmployeeData;
                    lvNewBills.DataBind();
            ...
            else
            {
                ...
                string IsVerified = objCommon.LookUp("ACC_ALL_TEMP_TRANS", "DISTINCT ISVerified", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_SQN=" + cmd.ToString().Trim());
                ...
                DataSet dsEmployeeData = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS_EMP_DATA", "*", "", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_SQN=" + cmd.ToString().Trim(), "");
                //temp edit
                if (dsEmployeeData != null && dsEmployeeData.Tables[0].Rows.Count > 0)
                {
                    ...
                    Response.Redirect("BulkVouchers.aspx?obj=configm," + cmd.ToString().Trim() + "," + lbvch.Text.ToString().Trim() + "," + txtFrmDate.Text.ToString().Trim() + "," + txtUptoDate.Text.ToString().Trim() + "," + txtAcc.Text.Split('*')[1].ToString().Trim() + "&pageno=" + Request.QueryString["pageno"]);

                }
                else
                {
                    string GSTApplicable = objCommon.LookUp("ACC_ALL_TEMP_TRANS", "IsGSTApplicable", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_SQN=" + cmd.ToString().Trim());
                    ...
                    else
                    {
                        ...
                        DataSet TF_ENTRY = objCommon.FillDropDown("ACC_ALL_TEMP_TRANS", "TRANSFER_ENTRY", "", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_SQN=" + cmd.ToString().Trim(), string.Empty);
                        if (IsVerified != "1" && Convert.ToInt32(TF_ENTRY.Tables[0].Rows[0][0].ToString()) == 0 || Convert.ToInt32(TF_ENTRY.Tables[0].Rows[0][0].ToString()) == 6)
                            Response.Redirect("AccountingVouchers.aspx?obj=configm," + cmd.ToString().Trim() + "," + lbvch.Text.ToString().Trim() + "," + txtFrmDate.Text.ToString().Trim() + "," + txtUptoDate.Text.ToString().Trim() + "," + txtAcc.Text.Split('*')[1].ToString().Trim() + "&pageno=" + Request.QueryString["pageno"]);
                        else
                        {
                            objCommon.DisplayUserMessage(UPDLedger, "Transferd Entry Can Not Be Modified .", this);
    ...
    protected void RptData_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            Label lblTempVoucher = (Label)e.Item.FindControl("lblTempVoucher");
            Label lbltemptest = (Label)e.Item.FindControl("lbltemptest");
            HtmlTableCell tdviewbill = (HtmlTableCell)e.Item.FindControl("tdviewbill");
            HtmlTableCell tdCopyVoucher = (HtmlTableCell)e.Item.FindControl("tdCopyVoucher");
            ...
            if (ViewState["Approved"] == "A")
            {
                btnEdit.Visible = false;
                btnDel.Visible = false;
                lbltemptest.Visible = true;
                lblTempVoucher.Visible = true;
                tdviewbill.Visible = true;
                tdCopyVoucher.Visible = false;
            ...
            else
            {
                ...
                btnDel.Visible = true;
                lbltemptest.Visible = false;
                lblTempVoucher.Visible = false;
                ViewState["Approved"] = null;
                tdviewbill.Visible = false;
                tdCopyVoucher.Visible = true;
            }
        }
    ...
    // ADDED BY TANU 18/12/2021  FOR APPROVAL VOUCHERS 

    public void showApprovedData()
    {
        lblMessage.Text = string.Empty;
        trGrid.Visible = true;
        if (Session["DatatableMod"] != null)
    ...
    private void AddApprovedGridEntry()
    {
        ...
        DataSet ds = objCommon.FillDropDown("ACC_" + Session["comp_code"].ToString() + "_trans", "*", "", "party_no=" + id.ToString().Trim() + " and transaction_type <> 'OB' and (ISApproved=" + 1 + " or ISVerified=" + 1 + " )    and CAST(transaction_date AS DATE) between '" + Fdate + "' and '" + Ldate + "'", "TRANSACTION_DATE,TRANSACTION_TYPE,VOUCHER_NO");
        if (ds == null)
        {
            objCommon.DisplayMessage("Record Not Available.", this);
            ClearRecord();
        ...
        if (ds != null)
        {
            DataSet dsvch = objCommon.FillDropDown("ACC_" + Session["comp_code"].ToString() + "_trans", "DISTINCT VOUCHER_NO as VOUCHER_NO,VOUCHER_SQN,TRANSACTION_DATE", "Transaction_Type,CHQ_NO,TEMP_VOUCHER_NO", "party_no=" + id.ToString().Trim() + " and transaction_type <> 'OB' and (ISApproved=" + 1 + " or ISVerified=" + 1 + ")  and CAST(transaction_date AS DATE) between '" + Fdate + "' and '" + Ldate + "'", "TRANSACTION_DATE,Transaction_Type,VOUCHER_NO");

            ViewState["Approved"] = "A";
            Session["VchDatatable"] = null;
            if (dsvch != null)
            {
                if (dsvch.Tables[0].Rows.Count != 0)
            ...
            DataSet dsOtran = new DataSet();
            dsOtran = objCommon.FillDropDown("(SELECT * FROM ACC_" + Session["comp_code"].ToString() + "_trans O WHERE O.TRANSACTION_TYPE+'_'+O.VOUCHER_NO IN (SELECT DISTINCT I.TRANSACTION_TYPE+'_'+I.VOUCHER_NO FROM ACC_" + Session["comp_code"].ToString() + "_trans  I WHERE I.PARTY_NO = '" + partyNo + "'   and (I.ISApproved=" + 1 + " or I.ISVerified=" + 1 + "))) a inner join ACC_" + Session["comp_code"].ToString() + "_PARTY AP on(a.PARTY_NO=ap.PARTY_NO)", "CONVERT(VARCHAR(8), a.TRANSACTION_DATE, 3) as Date,AP.PARTY_NAME as Particulars,CASE A.TRANSACTION_TYPE WHEN 'P'  THEN 'PAYMENT' WHEN  'R' THEN 'RECEIPT' WHEN 'C' THEN 'CONTRA' ELSE 'JOURNAL' END AS VchType,a.VOUCHER_NO as VchNo", "(case  A.[TRAN]	when  'Dr' then A.amount else 0 end) as DEBIT ,(case  A.[TRAN]	when  'Cr' then A.amount else 0 end) as CREDIT", "(a.ISApproved=" + 1 + " or a.ISVerified=" + 1 + ")  and CAST(a.transaction_date AS DATE) between '" + Fdate + "' and '" + Ldate + "'", "A.TRANSACTION_NO,A.VOUCHER_NO");
            dt = dsOtran.Tables[0];
            Session["DatatableMod"] = dt;
            dsOtran.Dispose();
        }
    ...
    {
        //Added by tanu  for multiple bill upload after approval
        DataTable dtD = null;
        if (ViewState["DOCS"] != null && ((DataTable)ViewState["DOCS"]) != null)
        {
        ...
        objATEnt.VOUCHER_NO = Convert.ToInt32(ViewState["NEW_VOUCHER_SQN"]);
        objATEnt.VOUCHER_SQN = Convert.ToInt32(objCommon.LookUp("ACC_" + Session["comp_code"].ToString() + "_trans", "DISTINCT  VOUCHER_SQN ", "VOUCHER_NO=" + Convert.ToInt32(ViewState["NEW_VOUCHER_SQN"])));

        int TEMP_VOUCHER_NOS = Convert.ToInt32(objCommon.LookUp("ACC_" + Session["comp_code"].ToString() + "_trans", "DISTINCT VOUCHER_NO", "VOUCHER_SQN=" + Convert.ToInt32(ViewState["NEW_VOUCHER_SQN"])));

        int TEMP_VOUCHER_NO = Convert.ToInt32(ViewState["OLD_VOUCHER_NO"]);
        if (dtD.Rows.Count > 0)
        {
            objATEnt.FILEPATH = Docpath + "VOUCHER_BILL\\EMPID_" + Convert.ToInt32(Session["userno"]) + "\\BillNo_" + TEMP_VOUCHER_NO;
            if (ViewState["DESTINATION_PATH"] != null)
            {
                AddDocuments(TEMP_VOUCHER_NO);
            }
        }
        else
        {
        ...
        string ISBulk = string.Empty;
        string FinancialDate = objCommon.LookUp("ACC_COMPANY", "cast(COMPANY_FINDATE_FROM as nvarchar(20))" + "+ ''' and ''' +" + "cast(COMPANY_FINDATE_TO as nvarchar(20))", "COMPANY_CODE='" + Session["comp_code"].ToString() + "'");
        DataSet dsEmployeeData = objCommon.FillDropDown("ACC_ALL_MAIN_TRANS_EMP_DATA", "*", "", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_NO=" + Convert.ToInt32(ViewState["NEW_VOUCHER_SQN"]), "");
        if (dsEmployeeData != null && dsEmployeeData.Tables[0].Rows.Count > 0)
        {
            ISBulk = "Y";
        }
        ...
        int ret = objPC1.Addbillmodification(objATEnt, Convert.ToInt32(Session["userno"].ToString()), dtD, Session["comp_code"].ToString().Trim(), Convert.ToInt32(Session["comp_no"].ToString().Trim()), ISBulk);
        ...
        {
           // AddDocuments(objATEnt.VOUCHER_SQN);
    ...
    protected void rdblistbill_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (rdblistbill.SelectedValue == "A")
        {
