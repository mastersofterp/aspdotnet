...
public partial class AccountingVouchersModifications : System.Web.UI.Page
{
    ...
    private void AddGridEntry()
    {
        ...
        if (ds != null)
        {
            ...
            dsOtran = objCommon.FillDropDown("(SELECT * FROM ACC_" + Session["comp_code"].ToString() + "_TRANS O WHERE O.TRANSACTION_TYPE+'_'+O.VOUCHER_NO IN (SELECT DISTINCT I.TRANSACTION_TYPE+'_'+I.VOUCHER_NO FROM ACC_" + Session["comp_code"].ToString() + "_TRANS  I WHERE I.PARTY_NO = '" + partyNo + "')) a inner join ACC_" + Session["comp_code"].ToString() + "_PARTY AP on(a.PARTY_NO=ap.PARTY_NO)", "CONVERT(VARCHAR(8), a.TRANSACTION_DATE, 3) as Date,AP.PARTY_NAME as Particulars,CASE A.TRANSACTION_TYPE WHEN 'P'  THEN 'PAYMENT' WHEN  'R' THEN 'RECEIPT' WHEN 'C' THEN 'CONTRA' ELSE 'JOURNAL' END AS VchType,a.VOUCHER_NO as VchNo", "(case  A.[TRAN]	when  'Dr' then A.amount else 0 end) as DEBIT,(case  A.[TRAN]	when  'Cr' then A.amount else 0 end) as CREDIT", "a.transaction_date between '" + Convert.ToDateTime(txtFrmDate.Text).ToString("dd-MMM-yyyy") + "' and '" + Convert.ToDateTime(txtUptoDate.Text).ToString("dd-MMM-yyyy") + "'", "A.TRANSACTION_NO,A.VOUCHER_NO");
            dt = dsOtran.Tables[0];
            Session["DatatableMod"] = dt;
            dsOtran.Dispose();
        }
        ...
        if (lbvch != null)
        {
                ...
                if (isFourSign == "N")
                {
                    if (lbvch.Text.ToString().Trim() == "Payment" || lbvch.Text.ToString().Trim() == "Receipt")
                    {
                       // ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucher1.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
                        ShowVoucherCashBankReport("Voucher", "PmtRcptCashVoucherRpt.rpt", lbvch.Text.ToString().Trim(), cmd, isBankCash);
            ...
            else
            {
                string FinancialDate = objCommon.LookUp("ACC_COMPANY", "cast(COMPANY_FINDATE_FROM as nvarchar(20))" + "+ ''' and ''' +" + "cast(COMPANY_FINDATE_TO as nvarchar(20))", "COMPANY_CODE='" + Session["comp_code"].ToString() + "'");
                  ...
                  DataSet dsEmployeeData = objCommon.FillDropDown("ACC_ALL_MAIN_TRANS_EMP_DATA", "*", "", "TRANSACTION_DATE BETWEEN '" + FinancialDate + "' and  VOUCHER_SQN=" + cmd.ToString().Trim(), "");
                //temp edit
