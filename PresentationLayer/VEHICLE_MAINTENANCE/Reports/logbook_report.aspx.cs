...
public partial class VEHICLE_MAINTENANCE_Reports_logbook_report : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ShowLogBookReport(rdoReportType.SelectedValue, "crylogBook_Report.rpt");
        }
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
    ...
    private void Clear()
    {
        ...
        ddlLogNo.SelectedValue = "0";
        txtPassenger.Text = string.Empty;
        txtFDate.Text = string.Empty;
        txtTDate.Text = string.Empty;
    ...
    public void Fill_DropDown()
    {
        ...
        objCommon.FillDropDownList(ddlLogNo, "VEHICLE_LOGBOOK", "LOGBOOKID", "LOGBOOKID AS NAME", "", "LOGBOOKID");
        objCommon.FillDropDownList(ddlTripType, "VEHICLE_TRIPTYPE", "TTID", "TRIPTYPENAME", "ACTIVE = 1", "TRIPTYPENAME");
    }
    ...
    private void ShowLogBookReport(string exporttype, string rptFileName)
    {
        try
        {
            ...
            url += "&filename=LogBookReport." + exporttype; // +ddlVehicle.SelectedItem.Text + "_" + ddlDriver.SelectedItem.Text + "." + rdoReportType.SelectedValue;
            url += "&path=~,Reports,VEHICLE_MAINTENANCE," + rptFileName;
            url += "&param=@P_VIDNO="+ ddlVehicle.SelectedValue +",@P_DNO="+ ddlDriver.SelectedValue +",@P_LOGBOOKID="+ ddlLogNo.SelectedValue +",@P_TTID="+ ddlTripType.SelectedValue +",@P_PASSENGER="+txtPassenger.Text.Trim();
