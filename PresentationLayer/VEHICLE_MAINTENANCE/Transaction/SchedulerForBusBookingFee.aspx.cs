...
//MODIFIED BY   :
//MODIFIED DATE :
//USED FOR      : TO SAVE EXAM FEE BILLDESK FAILED PAYMENT DETAILS
...
public partial class ACADEMIC_Automatic_Payment_Update : System.Web.UI.Page
{
    ...
    public int SubmitData(string Order_ID, int IDNO, string IsValid, string Receipt_Code, string TransactionDate, string TransactionID, string PayStatus, string Message,string TranDateTime)
    {
        ...
        try
        {
            ...
            object studid = objSqlhelper.ExecuteNonQuerySP("PKG_NON_ACD_INSERT_AUTO_UPDATED_BUS_TRANSACTION", sqlparam, true);
    ...
    public int UpdateFailData(string Order_ID, int IDNO)
    {
        ...
        try
        {
            ...
            object studid = objSqlhelper.ExecuteNonQuerySP("PKG_NON_ACD_UPDATE_FAIL_AUTO_TRANSACTION_CRESCENT", sqlparam, true);
            return retStatus1 = Convert.ToInt32(studid);
        }
        catch (Exception ex)
        {
    ...
    public DataSet GetAllDCRTempTrasaction()
    {
        ...
        try
        {
            ...
            ds = objSqlhelper.ExecuteDataSetSP("PKG_GET_BUS_TRANS_RECORD_FOR_AUTOSCHEDULAR", sqlParams);
        }
        catch (Exception ex)
        {
            return ds;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (dsAllTransactions != null && dsAllTransactions.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < dsAllTransactions.Tables[0].Rows.Count; i++)
            {
                ...
                dataNew = result;
                TranMesasge = result.ToString();                //NOT FETCH DATA AT THE SCHEDULER RUN 
