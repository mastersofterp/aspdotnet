...
// DESCRIPTION   : USED TO CHECK RECEIVED STATUS ON SENDING EMAIL FOR MOM
//========================================================================
using System;
using System.Data;
using System.Web;
...
public partial class MEETING_MANAGEMENT_TRANSACTION_CheckEmailStatusForMinutes : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "MEETING_MANAGEMENT_TRANSACTION_CheckEmailStatusForMinutes.Page_Load -> " + ex.Message + " " + ex.StackTrace);
            else
    ...
    private void BindlistView()
    {
        try
        {
            //  DataSet ds = objCommon.FillDropDown("TBL_MM_EMAIL_CONFIRMATION EC INNER JOIN TBL_MM_COMITEE C ON (EC.COMMITTEEID = C.ID) INNER JOIN TBL_MM_MENBERDETAILS MD ON (EC.MEMBER_USERID = MD.USERID)", "EC.ECID, C.NAME, EC.MEETING_CODE, MD.FNAME +' '+ MD.MNAME +' '+ MD.LNAME AS NAME", "(CASE EC.RECEIVE_STATUS WHEN 1 THEN 'Received' ELSE 'Not Received' END) AS RECEIVE_STATUS, RECEIVED_DATE", "EC.COMMITTEEID =" + Convert.ToInt32(ddlCommitee.SelectedValue) + " AND EC.MEETING_CODE ='" + Convert.ToInt32(ddlpremeeting.SelectedItem.Text) + "'", "EC.ECID");    + " AND EC.MEETING_CODE ='" + Convert.ToInt32(ddlpremeeting.SelectedItem.Text) + "'"
            DataSet ds = objCommon.FillDropDown("TBL_MM_EMAIL_CONFIRMATION EC INNER JOIN TBL_MM_COMITEE C ON (EC.COMMITTEEID = C.ID)  INNER JOIN TBL_MM_MENBERDETAILS MD ON (EC.FK_MEMBER = MD.PK_CMEMBER)", "ROW_NUMBER() OVER(ORDER BY EC.ECID ASC) AS Row, EC.ECID, C.NAME, EC.MEETING_CODE, MD.FNAME +' '+ (case MD.MNAME when '-' then '' else MD.MNAME end) +' '+ MD.LNAME AS MEMBER_NAME", "(CASE EC.DRAFT_RECEIVE_STATUS WHEN 1 THEN 'Received' ELSE 'Not Received' END) AS DRAFT_RECEIVE_STATUS, DRAFT_RECEIVE_DATE", "EC.COMMITTEEID =" + Convert.ToInt32(ddlCommitee.SelectedValue) + " AND EC.MEETING_CODE ='" + ddlpremeeting.SelectedItem.Text + "'", "EC.ECID");
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "MEETING_MANAGEMENT_TRANSACTION_CheckEmailStatusForMinutes.BindlistView -> " + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "MEETING_MANAGEMENT_TRANSACTION_CheckEmailStatusForMinutes.btnSubmit_Click -> " + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    ...
    protected void rdbCommitteeType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "MEETING_MANAGEMENT_TRANSACTION_CheckEmailStatusForMinutes.rdbCommitteeType_SelectedIndexChanged() --> " + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server Unavailable.");
        }
    }
