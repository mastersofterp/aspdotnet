...
// DESCRIPTION   : TO SAVE DESIGNATIONS.
//===========================================================
...
public partial class MEETING_MANAGEMENT_MASTER_DesigMaster : System.Web.UI.Page
{
    ...
    private void BindlistView()
    {
        try
        {
            DataSet ds = objCommon.FillDropDown("tbl_mm_CommiteeDesig", "*", "", "STATUS=0", "PK_COMMITEEDES");
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
        ...
        try
        {
            objMM.DESIGNAME = Convert.ToString(txtDesigname.Text.Replace("'","`"));
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int DesigID = int.Parse(btnDelete.CommandArgument);
            ViewState["DESIGNATION_ID"] = int.Parse(btnDelete.CommandArgument);

            DataSet ds = objCommon.FillDropDown("Tbl_MM_ReletionMaster", "*", "", "FK_COMMITEE_DESIG=" + DesigID, "");
            if (ds.Tables[0].Rows.Count > 0)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('This designation can not delete, it is in use.');", true);
                return;
            }
            else
            {
                CustomStatus cs = (CustomStatus)objMC.DeleteDesignation(DesigID);
    ...
    private void ShowDetails(int DesigNo)
    {
        try
        {
            DataSet ds = objCommon.FillDropDown("tbl_mm_CommiteeDesig", "*", "", "PK_COMMITEEDES=" + Convert.ToInt32(ViewState["DesigNo"]) + "", "PK_COMMITEEDES");
            if (ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    txtDesigname.Text = dr["DESIGNATION"].ToString().Replace("`","'");
                }
            }
        }
        catch (Exception ex)
