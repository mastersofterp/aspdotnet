...
// MODIFY DATE   : 16-FEB-2015
// DESCRIPTION   : INTRODUCE DEPNO TO SEPARATE COMMITTEES DEPARTMENTWISE.
// =============================================================
using System;
using System.Data;
using System.Web;
...
public partial class MEETING_MGT_Master_CommitteMaster : System.Web.UI.Page
{
    ...
    private bool checkCommitteeExist()
    {
        ...
        bool retVal = false;
        DataSet ds = objCommon.FillDropDown("Tbl_MM_COMITEE", "ID", "NAME", "NAME='" + txtname.Text + "'", "ID");
        if (ds != null)
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
           ...
           // objMM.DEPTNO = Convert.ToInt32(Session["UA_EmpDeptNo"]);
            ...
            if (ViewState["action"] != null)
            {
                ...
                //{
                    ...
                    {
                        DataSet ds = objCommon.FillDropDown("TBL_MM_COMITEE", "*", "", "STATUS = 0 AND (NAME ='" + objMM.NAME + "' OR CODE='" + objMM.CODE + "')" , "");
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                        ...
                        else
                        {
                            objMM.ID = 0;
                            ptype = 1;
                            CustomStatus cs = (CustomStatus)objMC.AddUpdate_Comittee_Details(objMM);
                    ...
                    else
                    {
                        ...
                        ptype = 2;
                        DataSet ds = objCommon.FillDropDown("TBL_MM_COMITEE", "*", "", "(NAME ='" + objMM.NAME + "' OR CODE='" + objMM.CODE + "') AND ID!=" + Convert.ToInt32(ViewState["CommitteeId"]), "");
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            objCommon.DisplayMessage(this.updActivity, "Record Already Exist.", this.Page);
                            txtname.Text = string.Empty;
    ...
    private void ShowDetails(int EDID)
    {
        try
        {
            DataSet ds = objCommon.FillDropDown("TBL_MM_COMITEE", "*", "", "ID=" + Convert.ToInt32(ViewState["CommitteeId"]) + "", "ID");
            if (ds.Tables[0].Rows.Count > 0)
            {
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataSet ds = objCommon.FillDropDown("Tbl_MM_ReletionMaster", "*", "", "FK_COMMITEE=" + committeeID, "");
            if (ds.Tables[0].Rows.Count > 0)
            {
            ...
            {
                CustomStatus cs = (CustomStatus)objMC.DeleteCommittee(committeeID);
