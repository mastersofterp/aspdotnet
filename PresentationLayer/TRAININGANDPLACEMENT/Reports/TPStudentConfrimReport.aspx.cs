...
public partial class TRAININGANDPLACEMENT_Reports_TPStudentConfrimReport : System.Web.UI.Page
{
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ShowReport("SelectionList", "TP_StudnetCompanyConfirm.rpt");
        }
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
    ...
    private void ShowReport(string reportTitle, string rptFileName)
    {
        try
        {
            ...
            url += "&param=@P_SCHEDULENO=" + Convert.ToInt32(ddlSchedule.SelectedValue)+ ",@P_COLLEGE_CODE="+Convert.ToInt32(Session["colcode"]);
