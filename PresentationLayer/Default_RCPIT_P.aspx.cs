...
public partial class Default_RCPIT_P : System.Web.UI.Page
{
    public class Packet
    {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    if (ds.Tables[0].Rows[0]["UA_STATUS"].ToString().Trim() == "0")
                    {
                        ...
                        try
                        {
                            ...
                            //status = task.Result;
                            status = objSendEmail.SendEmail(txtEmail.Text, message, subject);//Added BY rahul on dated 03062023
                        }
                        catch (Exception ex3)
                        {
...
StringComparison.OrdinalIgnoreCase) >= 0))
    ...
    public static List<Packet> GetData(string SLOGINDATA, string IDTOKEN)
    {
        ...
        Default_RCPIT_P objdefault = new Default_RCPIT_P();
        Common objCommon = new Common();
