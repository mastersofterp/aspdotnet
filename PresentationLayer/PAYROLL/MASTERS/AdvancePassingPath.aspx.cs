...
public partial class PAYROLL_MASTERS_AdvancePassingPath : System.Web.UI.Page
{
    //Creating objects of Class Files Common,UAIMS_COMMON,Advance Leave Controller
    Common objCommon = new Common();

    UAIMS_Common objUCommon = new UAIMS_Common();

    AdvancePassingAuthorityController objPassAuthCon = new AdvancePassingAuthorityController();
    ...
    protected void BindListViewPAPath()
    {
        try
        {
            DataSet ds = objPassAuthCon.GetAllPAPath(Convert.ToInt32(ddlCollege.SelectedValue));
        ...
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSINGPATH.BindListViewPAPath ->" + ex.Message + " " + ex.StackTrace);
            else
    ...
    private void FillDepartment()
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.Fill_Department ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    {
        try
        {//PAYROLL_LEAVE_PASSING_AUTHORITY
            objCommon.FillDropDownList(ddlPA01, "PAYROLL_ADVANCE_PASSING_AUTHORITY ", "PANO", "Passing_Authority_Name", "COLLEGE_NO=" + Convert.ToInt32(ddlCollege.SelectedValue) + "", "Passing_Authority_Name");
        }
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.FillPAuthority ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            // objAPAuth.LNO = 0;
            objAPAuth.DEPTNO = Convert.ToInt32(ddlDept.SelectedValue);
            ...
            {
                objAPAuth.PAN01 = Convert.ToInt32(ddlPA01.SelectedValue);
            }
            else
            {
                objAPAuth.PAN01 = 0;
            ...
            {
                objAPAuth.PAN02 = Convert.ToInt32(ddlPA02.SelectedValue);
            }
            else
            {
                objAPAuth.PAN02 = 0;
            ...
            {
                objAPAuth.PAN03 = Convert.ToInt32(ddlPA03.SelectedValue);
            }
            else
            {
                objAPAuth.PAN03 = 0;
            ...
            {
                objAPAuth.PAN04 = Convert.ToInt32(ddlPA04.SelectedValue);
            }
            else
            {
                objAPAuth.PAN04 = 0;
            ...
            {
                objAPAuth.PAN05 = Convert.ToInt32(ddlPA05.SelectedValue);
            }
            else
            {
                objAPAuth.PAN05 = 0;
            }
            objAPAuth.PAPATH = Convert.ToString(txtPAPath.Text);
            objAPAuth.COLLEGE_CODE = Convert.ToInt32(Session["colcode"]);
            ...
            if (trEmp.Visible == true) 
            {
                ...
                objAPAuth.PAPNO = Convert.ToInt32(ViewState["PAPNO"].ToString());
            ...
            else
            {
                foreach (ListViewItem lvitem in lvEmployees.Items)
                {
                    ...
                    {
                        count = count + 1;
                        objAPAuth.EMPNO = Convert.ToInt32(chkIdNo.ToolTip);
                        if (ViewState["action"].ToString().Equals("add"))
                        {
                            DataSet ds = objCommon.FillDropDown("PAYROLL_ADVANCE_PASSING_AUTHORITY_PATH", "PAPNO", "IDNO", "IDNO=" + objAPAuth.EMPNO + " ", "");
                            if (ds.Tables[0].Rows.Count <= 0)
                            {
                                count_record = count_record + 1;
                                DataRow dr = dtEmpRecord.NewRow();
                                dr["IDNO"] = objAPAuth.EMPNO;
                                dtEmpRecord.Rows.Add(dr);
                                dtEmpRecord.AcceptChanges();
                            }
                        ...
                        else
                        {
                            ...
                            objAPAuth.PAPNO = Convert.ToInt32(ViewState["PAPNO"].ToString());
                            DataSet ds = objCommon.FillDropDown("PAYROLL_ADVANCE_PASSING_AUTHORITY_PATH", "PAPNO", "IDNO", "IDNO=" + objAPAuth.EMPNO + " AND PAPNO!=" + objAPAuth.PAPNO + " ", "");
                            if (ds.Tables[0].Rows.Count <= 0)
                            {
                                count_record = count_record + 1;
                                DataRow dr = dtEmpRecord.NewRow();
            ...
            if (ViewState["action"] != null)
            {
                if (ViewState["action"].ToString().Equals("add"))
                {
                    ...
                    CustomStatus cs = (CustomStatus)objPassAuthCon.AddPAPath(objAPAuth, dtEmpRecord);
                ...
                else
                {
                    ...
                    {
                        objAPAuth.ADVCAHEDATE =Convert.ToDateTime(DateTime.Now.ToString("dd/MM/yyyy"));
                        objAPAuth.PAPNO = Convert.ToInt32(ViewState["PAPNO"].ToString());
                        // CustomStatus CS = (CustomStatus)objPAPath.UpdatePAPath(objLeaves);
                        CustomStatus CS = (CustomStatus)objPassAuthCon.UpdatePAPath(objAPAuth, dtEmpRecord);
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL__Master_ADVANCE_PASSING_Path.btnSave_Click ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    private void ShowDetails(Int32 PAPNO)
    {
        ...
        try
        {
            ds = objPassAuthCon.GetSinglePAPath(PAPNO);
            if (ds.Tables[0].Rows.Count > 0)
            {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.ShowDetails ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
        ...
        switch (index)
        {
            case 1:
                if (ddlPA01.SelectedIndex == 0)
                {
                    ...
                    objCommon.FillDropDownList(ddlPA02, "PAYROLL_ADVANCE_PASSING_AUTHORITY", "PANO", "Passing_Authority_Name", swhere, "Passing_Authority_Name");
            ...
            case 2:
                if (ddlPA02.SelectedIndex == 0)
                {
                    ...
                    objCommon.FillDropDownList(ddlPA03, "PAYROLL_ADVANCE_PASSING_AUTHORITY", "PANO", "Passing_Authority_Name", swhere, "Passing_Authority_Name");
            ...
            case 3:
                ...
                if (ddlPA03.SelectedIndex == 0)
                {
                    ...
                    objCommon.FillDropDownList(ddlPA04, "PAYROLL_ADVANCE_PASSING_AUTHORITY", "PANO", "Passing_Authority_Name", swhere, "Passing_Authority_Name");
                    txtPAPath.Text = ddlPA01.SelectedItem.ToString() + "->" + ddlPA02.SelectedItem.ToString();
                }
                else
                {
            ...
            case 4:
                ...
                if (ddlPA04.SelectedIndex == 0)
                {
                    ...
                    objCommon.FillDropDownList(ddlPA05, "PAYROLL_ADVANCE_PASSING_AUTHORITY", "PANO", "Passing_Authority_Name", swhere, "Passing_Authority_Name");
                    txtPAPath.Text = ddlPA01.SelectedItem.ToString() + "->" + ddlPA02.SelectedItem.ToString() + "->" + ddlPA03.SelectedItem.ToString();
                }
                else
                {
    ...
    protected void btnShowReport_Click(object sender, EventArgs e)
    {
        try
        {
            ShowReport("PAYROLL", "PAYROLL_PAPAth.rpt");
        }
        catch (Exception ex)
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCE_PASSING_PATH.btnShowReport_Click->" + ex.Message + ' ' + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    private void ShowReport(string reportTitle, string rptFileName)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCE_PASSING_PATH.ShowReport->" + ex.Message + ' ' + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void ddlPA01_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.ddlPA01_click ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void ddlPA02_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.ddlPA02_SelectedIndexChanged ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void ddlPA03_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.ddlPA03_SelectedIndexChanged ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void ddlPA04_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.ddlPA04_SelectedIndexChanged ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void ddlPA05_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_Master_ADVANCEPASSING_Path.ddlPA05_SelectedIndexChanged ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }

    protected void lvPAPath_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
    ...
    protected void btnDelete_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            CustomStatus cs = (CustomStatus)objPassAuthCon.DeletePAPath(PAPNO);
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_MASTER_ADVANCE_AUTHORITY_PATH.btnDelete_Click ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
    ...
    protected void btnEdit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            if (Convert.ToBoolean(Session["error"]) == true)
                objUCommon.ShowError(Page, "PAYROLL_MASTER_ADVANCE_AUTHORITY_PATH.btnEdit_Click ->" + ex.Message + " " + ex.StackTrace);
            else
                objUCommon.ShowError(Page, "Server UnAvailable");
        }
    }
