...
public partial class PayRoll_Pay_Monthly_Installment_Entry : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (Convert.ToInt32(txtNoofInstallMent.Text) < Convert.ToInt32(txtNoofInstPaid.Text))
            {
                objCommon.DisplayMessage(UpdatePanel1, "Total no of Installment should be grater than Paid Installment ", this);
                //Clear();    
                return;
            }
            if(Convert.ToDateTime(txtStartDate.Text) > Convert.ToDateTime(txtInstallmentDrawnDate.Text))
            {
                objCommon.DisplayMessage(UpdatePanel1, "Installment Start Date should be less than Installment  Drawn Date ", this);
                //Clear();    
                return;
            }
    ...
    private void FillEmployee()
    {
        try
        {
            if (ddlorderby.SelectedValue == "1")
                objCommon.FillDropDownList(ddlEmployee, "PAYROLL_EMPMAS EM,PAYROLL_PAYMAS PM", "EM.IDNO AS IDNO", " '['+ CONVERT(NVARCHAR(20),EM.PFILENO) +']'+ ISNULL(EM.FNAME,'')+' '+ISNULL(EM.MNAME,'')+' '+ISNULL(EM.LNAME,'') as ENAME", "EM.IDNO = PM.IDNO AND PM.PSTATUS='Y' and EM.IDNO > 0 AND EM.STATUS IS NULL AND EM.COLLEGE_NO="+ddlCollege.SelectedValue, "EM.IDNO");
    ...
    protected void txtNoofInstPaid_TextChanged(object sender, EventArgs e)
    {
        int MonthlyDeductedAmount =Convert.ToInt32(txtMonthlyDedAmt.Text);
        int TotalPaidInstallmentno = Convert.ToInt32(txtNoofInstPaid.Text);
        int TotalPaiAmount = MonthlyDeductedAmount * TotalPaidInstallmentno;

        int TotalAmount = Convert.ToInt32(txtTotalAmount.Text);
        int OutstandingAmount = TotalAmount - TotalPaiAmount;
        txtOutStandingAmt.Text = OutstandingAmount.ToString();      
    }
}
